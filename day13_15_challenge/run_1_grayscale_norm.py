...
def run_effnet_ts_grayscale_norm(day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir, custom_mean, custom_std):
    from torchvision.transforms import InterpolationMode
    set_seed_attn(SEED)
    out_dir = base_dir / "effnet_ts_grayscale_norm" / f"day_{str(day).replace('.', '_')}"
    ...
    train_ds = OrganoidTimeSeriesDatasetGrayscaleNorm(
        train_ids_f, series_metadata, data, custom_mean, custom_std,
        transform=train_tf, max_day=day, image_key="overlay_path",
    )
    val_ds = OrganoidTimeSeriesDatasetGrayscaleNorm(
        val_ids_f, series_metadata, data, custom_mean, custom_std,
        transform=eval_tf, max_day=day, image_key="overlay_path",
    )
    test_ds = OrganoidTimeSeriesDatasetGrayscaleNorm(
        test_ids_f, series_metadata, data, custom_mean, custom_std,
        transform=eval_tf, max_day=day, image_key="overlay_path",
    )

    train_loader = DataLoader(
        train_ds, batch_size=EFFNET_TS_BATCH, shuffle=True, num_workers=EFFNET_NUM_WORKERS or 0,
        collate_fn=collate_variable_length, pin_memory=(device.type == "cuda"),
    )
    val_loader = DataLoader(
        val_ds, batch_size=EFFNET_TS_BATCH, shuffle=False, num_workers=EFFNET_NUM_WORKERS or 0,
        collate_fn=collate_variable_length, pin_memory=(device.type == "cuda"),
    )
    test_loader = DataLoader(
        test_ds, batch_size=EFFNET_TS_BATCH, shuffle=False, num_workers=EFFNET_NUM_WORKERS or 0,
        collate_fn=collate_variable_length, pin_memory=(device.type == "cuda"),
    )
    ...
    for epoch in range(1, EFFNET_MAX_EPOCHS + 1):
        ...
        train_history.append({"epoch": epoch, "train_acc": train_acc, "val_acc": val_acc, "val_f1": val_f1})
    ...
    np.save(out_dir / "grayscale_mean_std.npy", {"mean": custom_mean, "std": custom_std})
    ...
    print(f"Day {day} effnet_ts (grayscale norm): test balanced_acc@0.5 = {test_at_05['balanced_acc']:.3f}")
    return save_result


def run_per_day_grayscale_norm(day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir, custom_mean, custom_std):
    set_seed_base(SEED)
    ...
    train_ds, _, _ = make_single_day_grayscale_norm_dataset(
        train_ids, val_ids, test_ids, series_metadata, data, day,
        custom_mean, custom_std, train_tf, image_key="overlay_path", use_rgb_mask=False,
    )
    _, val_ds, test_ds = make_single_day_grayscale_norm_dataset(
        train_ids, val_ids, test_ids, series_metadata, data, day,
        custom_mean, custom_std, eval_tf, image_key="overlay_path", use_rgb_mask=False,
    )
    ...
    with open(out_dir / "results.json", "w") as f:
        json.dump(save_result, f, indent=2)
    print(f"Day {day} per_day (grayscale norm): test balanced_acc@0.5 = {test_at_05['balanced_acc']:.3f}")
    return save_result
...
def main():
    ...
    for day in DAYS:
        ...
        if npy_path.exists():
            ...
            custom_mean, custom_std = compute_grayscale_mean_std(train_ids, series_metadata, data, day)
            np.save(npy_path, {"mean": custom_mean, "std": custom_std, "max_day": day})
        print(f"Day {day}: mean={custom_mean}, std={custom_std}")

        run_effnet_ts_grayscale_norm(day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir, custom_mean, custom_std)
        run_per_day_grayscale_norm(day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir, custom_mean, custom_std)
