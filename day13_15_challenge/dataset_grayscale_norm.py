...
class OrganoidTimeSeriesDatasetGrayscaleNorm(OrganoidTimeSeriesDataset):
    ...
    def __init__(self, organoid_ids, series_metadata, data, custom_mean, custom_std, **kwargs):
    ...
    def __getitem__(self, idx):
        ...
        for i, key in enumerate(entry_keys):
            ...
            if self.image_key == "overlay_path" and overlay_path and Path(overlay_path).exists():
                img = imread(overlay_path)
            elif self.image_key == "overlay_path" and mask_path and Path(mask_path).exists():
                img = imread(img_path)
                if img.ndim == 2:
            ...
            if self.transform:
                ...
                img_pil = Image.fromarray(
                    (ch3 * 255).astype(np.uint8) if ch3.max() <= 1 else ch3.astype(np.uint8)
                )
                ...
                if self.input_rgb_mask and img.shape[-1] == 4:
                    ...
                    m = img[:, :, 3:4]
                    m = zoom(m, (ch3.shape[0] / m.shape[0], ch3.shape[1] / m.shape[1], 1), order=0)
        ...
        if C == 3:
            ...
            mean_t = torch.tensor([0.485, 0.456, 0.406, 0.5], dtype=torch.float32).view(1, 4, 1, 1)
            std_t = torch.tensor([0.229, 0.224, 0.225, 0.5], dtype=torch.float32).view(1, 4, 1, 1)
...
def _single_day_getitem_grayscale_norm(self, idx):
    ...
    from skimage.io import imread
    from PIL import Image
    from analysis.images.cnn_lstm.train_base_model import _draw_outline_overlay, SingleDayOrganoidDataset
    sample = self.samples[idx]
    if self.image_key == "overlay_path" and sample.get("overlay_path") and Path(sample["overlay_path"]).exists():
        img = imread(sample["overlay_path"])
    elif self.image_key == "overlay_path" and sample.get("mask_path") and Path(sample["mask_path"]).exists():
    ...
    img = preprocess_stitched(img, sample["img_path"])
    img = img.astype(np.float32) / 255.0
    if self.use_rgb_mask and sample.get("mask_path") and Path(sample["mask_path"]).exists():
        mask = imread(sample["mask_path"])
        if mask.ndim == 3:
            mask = mask[:, :, 0]
        mask_bin = (mask > 127).astype(np.float32)
...
def make_single_day_grayscale_norm_dataset(train_ids, val_ids, test_ids, series_metadata, data, day, custom_mean, custom_std, transform, image_key="overlay_path", use_rgb_mask=False):
    ...
    train_ds = _SingleDayGrayscaleNorm(
        train_ids, series_metadata, data, day, transform=transform,
        image_key=image_key, use_rgb_mask=use_rgb_mask,
        custom_mean=mean_3, custom_std=std_3,
    )
    val_ds = _SingleDayGrayscaleNorm(
        val_ids, series_metadata, data, day, transform=transform,
        image_key=image_key, use_rgb_mask=use_rgb_mask,
        custom_mean=mean_3, custom_std=std_3,
    )
    test_ds = _SingleDayGrayscaleNorm(
        test_ids, series_metadata, data, day, transform=transform,
        image_key=image_key, use_rgb_mask=use_rgb_mask,
        custom_mean=mean_3, custom_std=std_3,
    )
