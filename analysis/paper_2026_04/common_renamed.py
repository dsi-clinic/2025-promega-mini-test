...
Revised version of ``common.py``. 
Currently only called in``metabolites_train.py``.
"""
...
def compute_classification_metrics(y_true, y_pred, y_prob=None) -> dict:
    ...
    Internal label convention (matches LABEL_TO_INT, used for sklearn calls here):
        0 = Acceptable, 1 = Not Acceptable.

    Paper reporting convention (used in the returned dict keys):
        Acceptable is the positive class. So paper TPR / sensitivity =
        Acceptable recall, and paper TNR / specificity = Not Acceptable recall.

    This function runs sklearn under the internal convention and then translates
    the results into the paper convention before returning, so a downstream
    reader can take ``recall_acceptable`` / ``recall_not_acceptable`` /
    ``sensitivity`` / ``specificity`` at face value.

    Note: ``confusion_matrix`` and ``n_positive`` / ``n_negative`` are reported
    under the internal convention (positive=1=Not Acceptable). They are raw
    counts and re-labelling them would be more confusing than helpful.
    """
    ...
    # sklearn ran with labels=[0, 1] where 1 = Not Acceptable. So:
    #   tp = NA correctly predicted, tn = Acceptable correctly predicted.
    # Paper convention treats Acceptable as positive, so we re-name accordingly.
    ...
    out = {
        ...
        # sklearn precision_recall_fscore_support was called with labels=[0, 1],
        # so index 0 corresponds to label 0 = Acceptable, index 1 to NA.
