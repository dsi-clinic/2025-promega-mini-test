    ...
    for days in unique_days:
        ...
        for col in X_train.columns:
            if nan_counts_train[col] > 0:
                print(f"    {col}: {nan_counts_train[col]}/{len(X_train)} "
                      f"({100*nan_counts_train[col]/len(X_train):.1f}%)")
        ...
        print(f"  Scaling complete. Mean={X_train_scaled.values.mean():.4f}, "
              f"Std={X_train_scaled.values.std():.4f}")
        ...
        if "Acceptable" in classes_order:
            ...
            y_score = proba[:, acc_idx]  # PROB(ACCEPTABLE)
        else:
        ...
        # ===== FIXED ROC-AUC: explicitly set Acceptable as positive =====
...
def main():
    ...
    variants = [
        ("modified_lgbm", "gbdt", False),
        ("modified_lgbm_gbdt_thresh", "gbdt", True),
        ("modified_lgbm_dart", "dart", False),
        ("modified_lgbm_dart_thresh", "dart", True),
    ]
