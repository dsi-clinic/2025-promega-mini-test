...
TARGET_SIZE = (384, 384) # ENHANCED: Increased from 224x224 for better feature extraction
...
# --- 7. Define Focal Loss for better hard example handling ---
...
# --- 8. Define a CNN model with Enhanced Pre-trained base ---
# ENHANCED: Using EfficientNetB3 instead of ResNet50V2 for better performance
# Using B3 instead of B7 to fit in memory and avoid shape issues
# NOTE: Training from scratch due to shape compatibility issues with pretrained weights
print("Loading EfficientNetB3 backbone (from scratch)...")
# Force input shape to be 3-channel RGB to match ImageNet weights
input_shape_rgb = (IMG_SHAPE[0], IMG_SHAPE[1], 3)
print(f"Using input shape: {input_shape_rgb}")
base_model = EfficientNetB3(include_top=False, weights=None, input_shape=input_shape_rgb)  # weights=None for scratch training
base_model.trainable = True # Trainable from the start since no pretrained weights

# Create separate input layers for the image and the mask
input_image = keras.Input(shape=input_shape_rgb)
...
# ENHANCED: Compile with focal loss instead of binary_crossentropy
model.compile(
    optimizer='adam', 
    loss=focal_loss(gamma=2.0, alpha=0.25),  # ENHANCED: Focal loss for hard examples
    metrics=[
        'accuracy',
...
print("\n--- Initial Model Summary (Training from Scratch) ---")
model.summary()
print("------------------------------------------")

# --- 9. Define Enhanced Callbacks ---
...
# ENHANCED: Add learning rate reduction on plateau
...
# ENHANCED: Add model checkpoint to save best model
checkpoint = ModelCheckpoint(
    'organoid_classifier_best_model_tony_phase1.h5',
    monitor='val_auc',
    save_best_only=True,
    mode='max',
    verbose=1
...
# --- 10. Train the model (Phase 1: Training from Scratch) ---
epochs_phase1 = 100 # Train from scratch with more epochs

print(f"\n--- Training Phase 1: Training from Scratch with Augmentation ({epochs_phase1} epochs) ---")
print("ENHANCED: Using EfficientNetB3 (from scratch) + Focal Loss + LR Scheduling")
history = model.fit(
    train_dataset, # Use the TF Dataset here
    epochs=epochs_phase1,
    validation_data=val_dataset, # Use the TF Dataset here
    callbacks=[early_stopping, reduce_lr, checkpoint],  # ENHANCED: Added callbacks
    class_weight=class_weights # Apply class weights here
)
print("----------------------------------------------------------")

# --- 11. Unfreeze and Fine-tune (Phase 2 with Data Augmentation) ---
print("\n--- Training Phase 2: Unfreezing and Fine-tuning Base Model with Augmentation ---")
print("ENHANCED: Unfreezing EfficientNetB7 + Focal Loss + LR Scheduling")
...
model.compile(
    optimizer=keras.optimizers.Adam(learning_rate=1e-4),  # ENHANCED: Lower LR for fine-tuning
    loss=focal_loss(gamma=2.0, alpha=0.25),  # ENHANCED: Keep focal loss
    metrics=[
        'accuracy',
        tf.keras.metrics.AUC(name='auc'),
        tf.keras.metrics.Precision(name='precision'),
...
# ENHANCED: Create callbacks for Phase 2 as well
checkpoint_phase2 = ModelCheckpoint(
    'organoid_classifier_best_model_tony_phase2.h5',
    monitor='val_auc',
    save_best_only=True,
    mode='max',
    verbose=1
...
history_fine_tune = model.fit(
    ...
    validation_data=val_dataset, # Use the TF Dataset here
    callbacks=[early_stopping_fine_tune, reduce_lr, checkpoint_phase2],  # ENHANCED: Added callbacks
...
model.save('organoid_classifier_final_model_tony.h5')
print("\nFinal model classifier saved as 'organoid_classifier_final_model_tony.h5'")
...
plt.savefig('training_auc_tony.png')
...
plt.savefig('training_loss_tony.png')

print("\nTraining history plots saved as 'training_auc_tony.png' and 'training_loss_tony.png'")
