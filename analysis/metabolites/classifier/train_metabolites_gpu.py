...
class Logger:
    ...
    def startup_block(self, config):
        ...
        print(f"  Search iters: {config['n_iter']} | search_n_jobs: {config['search_n_jobs']}")
    ...
    def check_fold_minority(self, y, groups, n_folds, day):
        ...
        try:
            ...
            if min_fold_minority < MIN_FOLD_MINORITY:
                self.warn("003", f"{day}: fold has only {min_fold_minority} minority samples (< {MIN_FOLD_MINORITY})")
    ...
    def check_smote_safety(self, minority_count, k_neighbors, day):
        ...
        if minority_count < k_neighbors + 1:
            self.warn("007", f"{day}: minority ({minority_count}) < k_neighbors+1 ({k_neighbors+1}) for SMOTE")
            return False
        return True
...
def focal_loss_lgbm(gamma=2.0, alpha=0.25):
    ...
    def focal_loss_objective(y_true, y_pred):
        ...
        grad = p - y_true + alpha * gamma * (1 - p) ** gamma * p * (
            gamma * p * np.log(np.maximum(p, 1e-15)) / (1 - p + 1e-15) - 1
        )
        hess = p * (1 - p) * (1 + alpha * gamma * (1 - p) ** (gamma - 1) * (
            gamma * p - 1
        ))
...
def tune_threshold(y_true_bin, y_prob, scoring):
    ...
    for t in thresholds:
        ...
        if scoring == "f1_notaccept":
            ...
            score = recall_score(y_true_bin, y_pred_bin, pos_label=0, zero_division=0)
        elif scoring == "f1_weighted":
            score = f1_score(y_true_bin, y_pred_bin, average="weighted", zero_division=0)
        elif scoring == "macro_f1":
...
def build_model(imbalance, y_train):
    ...
    if imbalance == "class_weight":
        ...
        model = LGBMClassifier(
            random_state=SEED, verbose=-1, n_jobs=n_jobs, device=device,
            objective=focal_loss_lgbm(gamma=2.0, alpha=0.25),
            boosting_type="gbdt"
        )
        return model, False
...
def train_metabolite_classifier_per_day(
    ...
    n_iter=None, search_n_jobs=1, day_filter=None
):
    ...
    for days in unique_days:
        ...
        # Clean data
        if imbalance == "smote":
            X_trainval_clean, _, X_test_clean, _ = clean_and_scale_data(X_trainval, X_test=X_test)
            # Check SMOTE safety
        ...
        for fold_idx, (tr_idx, va_idx) in enumerate(cv.split(X_trainval_clean, y_trainval, groups=groups_trainval)):
            ...
            inner_cv = StratifiedGroupKFold(n_splits=INNER_FOLDS, shuffle=True, random_state=SEED)
            scoring_fn = get_scoring_function(scoring)
            
            # RandomizedSearchCV for hyperparameter search
            search = RandomizedSearchCV(
                model, param_dist, n_iter=search_iterations,
                scoring=scoring_fn, cv=inner_cv, n_jobs=search_n_jobs,
            ...
            try:
                ...
                model.fit(X_tr, y_tr)
                search = type('obj', (object,), {'best_estimator_': model, 'best_score_': 0, 'best_params_': {}})()
            ...
            try:
                ...
                oof_proba = cross_val_predict(
                    ...
                    cv=inner_cv, method='predict_proba', n_jobs=search_n_jobs
                )
                # Get index for "Acceptable" class
                actual_model_inner = best_model.named_steps["model"] if is_pipeline else best_model
                ...
                threshold, _ = tune_threshold(y_tr_bin, oof_prob_acceptable, scoring)
            except Exception as e:
                print(f"    Fold {fold_idx}: OOF threshold tuning failed ({e}), using 0.5")
                threshold = 0.5
            ...
            if hasattr(best_model, 'predict_proba'):
                ...
                classes_order = list(best_model.classes_) if hasattr(best_model, 'classes_') else (
                    list(best_model.named_steps["model"].classes_) if is_pipeline else ["Not Acceptable", "Acceptable"]
                )
            ...
            if scoring == "f1_notaccept":
                ...
                fold_score = recall_score(y_va_bin, y_pred_va, pos_label=0, zero_division=0)
            elif scoring == "f1_weighted":
                fold_score = f1_score(y_va_bin, y_pred_va, average="weighted", zero_division=0)
            else:  # macro_f1
                fold_score = f1_score(y_va_bin, y_pred_va, average="macro", zero_division=0)
            ...
            logger.debug(f"{days} Fold {fold_idx}: thr={threshold:.3f}, score={fold_score:.3f}")
        ...
        logger.debug(f"{days}: CV mean={mean_cv_score:.3f}, median_thr={final_threshold:.3f}")
        if std_threshold > 0.15:
            logger.warn("011", f"{days}: threshold unstable (std={std_threshold:.3f} > 0.15)")
        ...
        y_pred_test = np.where(y_score_test >= final_threshold, "Acceptable", "Not Acceptable")
...
def main():
    ...
    model_name = f"lgbm_{args.scoring}_{args.imbalance}_gpu"
