...
def train_metabolite_classifier_per_day(
    trainval, test_df, output_dir, model_name="lgbm"
):
    """
    ...
    for days in unique_days:
        ...
        # Prepare train data
        train_data = selected_train_df.drop(
            columns=[c for c in cols_to_drop if c in selected_train_df.columns]
        )
        test_data = selected_test_df.drop(
            columns=[c for c in cols_to_drop if c in selected_test_df.columns]
        )
        ...
        print("  NaN counts in train features:")
        for col in X_train.columns:
            if nan_counts_train[col] > 0:
                print(
                    f"    {col}: {nan_counts_train[col]}/{len(X_train)} ({100 * nan_counts_train[col] / len(X_train):.1f}%)"
                )
        ...
        assert not X_train.isna().any().any(), (
            "Training data still has NaNs after cleaning!"
        )
        ...
        LGBMClassifier(random_state=SEED, verbose=-1, n_jobs=1, scale_pos_weight=ratio)
        pipe = Pipeline(
            steps=[
        ...
        if len(np.unique(y_test)) > 1:
            ...
            for t in thresholds:
                ...
                f1_notacc = f1_score(
                    y_true_bin, y_bin_pred, pos_label=1, zero_division=0
                )
        ...
        report = classification_report(
            y_test, y_pred_thresh, output_dict=True, zero_division=0
        )
        ...
        different_rows = selected_test_df[selected_test_df["label"].values != y_pred]
    ...
    axes[1, 1].plot(
        summary_df["Day_No"],
        summary_df["Test_Recall_NotAcceptable"],
        "o-",
        color="purple",
    )
