...
def json_to_df(json_data):
    ...
    conc_cols = [c for c in df.columns if 'concentration' in c or 'Glo' in c or c in ['glucose', 'glutamate']]
...
def compute_growth_features(df):
    ...
    for met in metabolites:
        col_name = f'{met}_concentration_uM' if met != 'glucose' and met != 'glutamate' else met
...
def extract_feature_importance_for_day(train_df, val_df, day_str, output_dir):
    ...
    X_combined, y_combined, _ = prepare_data_for_day(day_combined, day_num, cols_to_drop_base)
    ...
    plt.title(f'Top {top_n} Feature Importances - {day_str}', fontweight='bold', fontsize=14)
    ...
    if len(importance_df) <= 50:
        ...
        plt.xlabel('Importance (split count)', fontweight='bold')
        plt.ylabel('Feature', fontweight='bold')
        plt.title(f'All Feature Importances - {day_str}', fontweight='bold', fontsize=14)
        plt.gca().invert_yaxis()
        plt.grid(axis='x', alpha=0.3)
        plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description="Extract Feature Importance from Metabolite Models")
    parser.add_argument("--days", nargs='+', default=['24', '28', '30'],
    ...
    if args.days == ['all']:
        ...
        day_mapping = {str(d): f for f in np.unique(train_df.DY) for d in [int(re.search(r'\d+', f).group())] if str(d) in args.days}
    ...
    for day_str in unique_days:
        importance_df = extract_feature_importance_for_day(train_df, val_df, day_str, output_dir)
        ...
        for feature in all_features:
            ...
            for day_str, imp_df in all_importance.items():
                ...
                if len(feature_row) > 0:
                    row[f'Day_{day_num}'] = feature_row['importance_normalized'].values[0]
                else:
