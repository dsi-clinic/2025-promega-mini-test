...
def train_trajectory_classifier_per_day(
    ...
    imbalance, scoring, all_days_sorted, feature_cols,
):
    ...
    for target_day in unique_days:
        ...
        if len(np.unique(y_val_arr)) > 1:
            ...
            print(f"  {day_label}: threshold={threshold:.3f}, {scoring}={thresh_score:.3f}")
    ...
    for target_day in unique_days:
        ...
        plt.title(f"Top {top_n} Feature Importances - {day_label} (trajectory)", fontweight="bold", fontsize=14)
        ...
        save_organoid_predictions(
            ids_test, y_test_arr, y_pred_test, y_score_na,
    ...
    print(f"{'=' * 60}")
    print(summary_df[["Day", "N_Features", "Test_F1_NotAcceptable", "Test_Recall_NotAcceptable",
    ...
    all_features = set()
    for target_day in unique_days:
        if target_day in per_day_info and "feature_importance_df" in per_day_info[target_day]:
    ...
    for feature in all_features:
        ...
        for target_day in unique_days:
            if target_day not in per_day_info or "feature_importance_df" not in per_day_info[target_day]:
                continue
            feat_df = per_day_info[target_day]["feature_importance_df"]
...
def main():
    ...
    feature_cols = [c for c in df_all.columns
                    if c not in metadata_cols and df_all[c].dtype != "O"
