...
def analyze_results():
    """Analyze metabolite classifier results and generate comparison reports.

    Aggregates results from outputs_metabolites directory, creates comparison plots,
    and generates summary tables with average metrics across days.
    """
    ...
    for metric_col, metric_name in metrics:
        ...
        for variant in combined_df["Model_Variant"].unique():
            subset = combined_df[combined_df["Model_Variant"] == variant].sort_values(
                "Day_No"
            )
            plt.plot(
                subset["Day_No"],
    ...
    avg_metrics = (
        combined_df.groupby("Model_Variant")[
            [
