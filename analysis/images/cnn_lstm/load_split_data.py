...
def load_split_data(train_split_path, val_split_path, test_split_path):
    ...
    def get_label_stats(ids):
        labels = [
            series_metadata[oid].get("label", "").lower()
            for oid in ids
            if oid in series_metadata
        ]
        acceptable = sum(1 for l in labels if l in ("acceptable", "good", "accepted"))
        not_acceptable = sum(
            1 for l in labels if l in ("not acceptable", "bad", "rejected", "not_good")
        )
...
if __name__ == "__main__":
    # Test loading — uses repo root relative to this file; or set base_dir to your data_splits path
    ...
    print(
        f"  Entry keys: {sample_meta['entry_keys'][:3]}... ({len(sample_meta['entry_keys'])} total)"
    )
