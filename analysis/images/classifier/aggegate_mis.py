...
def load_global_gt_from_jsons(json_paths: List[Path]) -> pd.DataFrame:
    ...
    if not rows:
        sys.exit("[error] Found Dy*.json files but none had usable (img_path, label) records.")
    gt = pd.DataFrame(rows).drop_duplicates("img_path")
    print(f"[info] Loaded global GT from {len(json_paths)} JSON file(s): {len(gt)} unique images")
    return gt
...
def main(input_dir: Path, search_root: Path):
    ...
    if not json_paths:
        sys.exit(f"[error] No Dy*.json found under {search_root.resolve()} "
    ...
    if inter_frames:
        ...
        print("[info] No Dy*_misclassified_by_all_models.csv files found; intersection not created.")
...
if __name__ == "__main__":
    ...
    ap.add_argument("-i", "--input-dir", default=str(DEFAULT_INPUT_DIR),
                    help=f"Folder with Dy*_misclassified_*.csv (default: {DEFAULT_INPUT_DIR})")
    ap.add_argument("-r", "--gt-search-root", default=str(DEFAULT_SEARCH_ROOT),
