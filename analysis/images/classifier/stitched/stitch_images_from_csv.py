...
def group_paths_by_label(records: List[dict]) -> Tuple[List[Tuple[str, Path]], List[Tuple[str, Path]]]:
...
def draw_captioned_tile(img: Image.Image, caption: str, font: ImageFont.ImageFont, caption_height: int, padding: int = 4) -> Image.Image:
...
def stitch_grid(items: List[Tuple[str, Path]], out_path: Path, cols: int, rows: int, caption_height: int, spacing: int) -> None:
    if not items:
        return
...
def chunk(items: List[Tuple[str, Path]], max_items: int) -> List[List[Tuple[str, Path]]]:
    return [items[i : i + max_items] for i in range(0, len(items), max_items)]


def stitch_category(items: List[Tuple[str, Path]], out_dir: Path, title: str, cols: int, rows: int, caption_height: int, spacing: int) -> List[Path]:
    out_paths = []
    max_items = cols * rows
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Stitch images by label from CSV with captions.")
    parser.add_argument("--csv", dest="csv_path", required=True, help="Path to aggregated_misclassified_by_all_models.csv")
    parser.add_argument("--outdir", dest="out_dir", required=False, default="analysis/images/classifier/stitched", help="Output directory for stitched images")
    ...
    parser.add_argument("--caption-height", type=int, default=18, help="Caption height in pixels under each tile")
    parser.add_argument("--spacing", type=int, default=6, help="Spacing in pixels between tiles")
    ...
    accepted_outs = stitch_category(accepted, out_dir, "Accepted", args.cols, args.rows, args.caption_height, args.spacing)
    not_accepted_outs = stitch_category(not_accepted, out_dir, "NotAccepted", args.cols, args.rows, args.caption_height, args.spacing)
