...
PLOT_DIR    = Path("outputs_efficientnet_zoom/both_plots_0.5")
BATCH_SIZE  = 16
TARGET_SIZE = (224, 224)
...
def epoch_loop(model, loader, optim, weights, train=True):
    ...
    for img, mask, lbl in loader:
        ...
        loss = bce(logit, lbl)
        w = torch.tensor([weights[int(y.item())] for y in lbl],
                         device=lbl.device)
        loss = (loss * w).mean()
        if train:
...
def main():
    ...
    X_tr, X_val, M_tr, M_val, y_tr, y_val = train_test_split(
        imgs, masks, labels, test_size=0.2,
    ...
    train_loader = make_loader(
        X_tr, M_tr, y_tr, augment=True,  batch=BATCH_SIZE, zoom_scale=(0.5, 1.0))
    val_loader   = make_loader(
        X_val, M_val, y_val, augment=True, batch=BATCH_SIZE,zoom_scale=(0.5, 1.0))
    ...
    h, best = {"train_f1": [], "val_f1": [],
               "train_loss": [], "val_loss": []}, -np.inf
    ...
    for ep in range(1, 200):
        ...
        h["train_f1"].append(tf1);  h["val_f1"].append(vf1)
        print(f"[P1][{ep:02d}] loss {tl:.4f}/{vl:.4f}  "
              f"f1 {tf1:.3f}/{vf1:.3f}")
        if vf1 > best:
            best = vf1
    ...
    for ep in range(1, 300):
        ...
        h["train_loss"].append(tl); h["val_loss"].append(vl)
        h["train_f1"].append(tf1);  h["val_f1"].append(vf1)
        print(f"[P2][{ep:03d}] loss {tl:.4f}/{vl:.4f}  "
              f"f1 {tf1:.3f}/{vf1:.3f}")
        if vf1 > best:
            best = vf1
            torch.save(model.state_dict(), MODEL_OUT)
    ...
    plt.title("Weighted F1"); plt.legend()
    ...
    with torch.no_grad():
        for img, mask, lbl in val_loader:
            probs = torch.sigmoid(
                model(img.to(DEVICE), mask.to(DEVICE))).cpu().numpy()
