...
def extract_auc_data():
    ...
    base_dir = Path("/net/scratch/jiaweizhang/2025-promega-mini-test/analysis/images/classifier/outputs_512x384_softlabels/soft_resnet")
    ...
    for day_dir in sorted(day_dirs, key=lambda x: extract_day_number(x.name)):
        ...
        if metrics_file.exists():
            with open(metrics_file, 'r') as f:
                ...
                print(f"Day {data['day']}: day_no={data['day_no']}, roc_auc={data['roc_auc']:.4f}")
...
def plot_auc_curve(days, aucs):
    ...
    # Create the plot
    plt.plot(actual_days, aucs, 'o-', linewidth=2, markersize=8, color='#2E86AB', markerfacecolor='#A23B72')
    
    # Customize the plot
    plt.xlabel('Day', fontsize=14, fontweight='bold')
    plt.ylabel('ROC AUC', fontsize=14, fontweight='bold')
    plt.title('ROC AUC Performance Across Different Days', fontsize=16, fontweight='bold', pad=20)
    ...
    for i, (day, auc) in enumerate(zip(actual_days, aucs)):
        plt.annotate(f'{auc:.3f}', (day, auc), textcoords="offset points", 
