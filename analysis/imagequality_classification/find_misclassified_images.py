...
def evaluate_fullset_and_write_csv(model, all_imgs, all_labels, csv_path):
    ...
    with csv_path.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["img_path", "true_label", "pred_prob", "pred_label"])
        writer.writeheader()
...
def run_training_for_day(day_json_path: Path, backbone_key: str, backbone_name: str,
    ...
    print(f"❌ Saved misclassified list (fullset) → {mis_csv_path}  (count={len(mis_set)})")
        ...
        if len(per_backbone) == len(BACKBONES):
            ...
            with inter_csv.open("w", newline="") as f:
                ...
                for pth in sorted(inter):
                    writer.writerow([pth])
            print(f"🤝 Saved intersection (all three models wrong) → {inter_csv}  (count={len(inter)})")
        else:
            print(f"ℹ️ Skipped intersection for {day} (not all models produced results).")
