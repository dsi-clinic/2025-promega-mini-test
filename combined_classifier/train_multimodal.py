...
class OrganoidDataset(Dataset):
    ...
    def __init__(self, df, config, transform=None, scaler=None, fit_scaler=False):
        ...
        self.img_key = (
            "overlay_path" if "overlay" in config["input_mode"] else "img_path"
        )
        ...
        if self.use_metabolites:
            self.meta_features_list = (
                self._extract_metabolite_features()
            ...
            if fit_scaler:
                ...
                max_dim = (
                    max(len(f) for f in self.meta_features_list)
                    if self.meta_features_list
                    else 5
                )
                padded_features = [
                    f + [0.0] * (max_dim - len(f)) for f in self.meta_features_list
                ]
                ...
                padded_features = [
                    f + [0.0] * (scaler_dim - len(f)) for f in self.meta_features_list
                ]
    ...
    def __getitem__(self, idx):
        ...
        if self.config["use_images"]:
            ...
            if self.use_mask:
                ...
                mask = T.Compose([T.Resize(self.config["target_size"]), T.ToTensor()])(
                    mask
                )
                items.append(mask)
...
class MetaboliteBranch(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.shape[1] < self.input_dim:
            padding = torch.zeros(
                x.shape[0], self.input_dim - x.shape[1], device=x.device
            )
...
class MultimodalClassifier(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.backbone = timm.create_model(
            backbone_name, pretrained=True, num_classes=0, **extra
        )
        ...
        self.head = nn.Sequential(
            nn.Linear(fused_dim, 128), nn.ReLU(), nn.Dropout(0.5), nn.Linear(128, 1)
        )
...
def pretrain_shared_backbone(train_df, val_df, config):
    ...
    labels = [
        train_ds.label_map.get(train_ds.df.iloc[i]["label"], 0)
    ...
    for epoch in range(config["num_epochs_phase1"]):
        ...
        if epoch % 10 == 0:
            print(
                f"Ep {epoch}: loss={tl:.4f}, tr_acc={ta:.3f}, val_acc={vr['acc']:.3f}"
            )
...
def train_for_day(
    ...
    # Class weights
    labels = [
        train_ds.label_map.get(train_day.iloc[i]["label"], 0)
    ...
    if shared_backbone_state is not None:
        ...
        pretrained_dict = {
            k: v
            for k, v in shared_backbone_state.items()
            if k in model_dict
            and (k.startswith("backbone.") or k.startswith("meta_branch."))
        }
    ...
    try:
        auc_f = float(auc)
        auc_str = "N/A" if np.isnan(auc_f) else f"{auc_f:.3f}"
        ...
        json.dump(
            {
                ...
                "test_pr_auc": float(test_res["pr_auc"])
                if test_res["pr_auc"]
                else None,
...
def main():
    ...
    # Model selection
    parser.add_argument(
        "--backbone", choices=["vit", "resnet", "efficientnet"], default="vit"
    )
    parser.add_argument(
        "--input-mode",
        choices=["rgb", "overlay", "rgb_mask", "overlay_mask"],
        default="rgb",
    )
    parser.add_argument(
        "--fusion-strategy",
    ...
    parser.add_argument(
        "--images-only", action="store_true", help="Use only images (no metabolites)"
    )
    parser.add_argument(
        "--metabolites-only",
        action="store_true",
        help="Use only metabolites (no images)",
    )
    ...
    parser.add_argument(
        "--output-dir", default="analysis/multimodal/outputs_multimodal"
    )
    ...
    shared_backbone_state, shared_scaler = pretrain_shared_backbone(
    ...
    if results:
        ...
        model_id = (
            f"{config['backbone']}_{config['input_mode']}_{config['fusion_strategy']}"
        )
        ...
        if summary["Test_ROC_AUC"].notna().any():
            axes[2].plot(
                summary["Day_No"], summary["Test_ROC_AUC"], "o-", color="green"
            )
