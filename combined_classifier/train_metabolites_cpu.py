...
class Logger:
    ...
    def startup_block(self, config):
        ...
        if grid_n_jobs != 1 and lgbm_n_jobs != 1:
            self.warn(
                "008",
                f"Oversubscription risk: GridSearchCV n_jobs={grid_n_jobs}, LightGBM n_jobs={lgbm_n_jobs}",
            )
        print(f"{'=' * 65}\n")
...
def build_model(imbalance, y_train):
    ...
    device = "cpu"
    n_jobs = (
        1  # Single thread to avoid oversubscription (GridSearchCV handles parallelism)
    )
...
def train_metabolite_classifier_per_day(
    ...
    if len(pr_auc_data) > 0:
        axes[1, 1].plot(
            pr_auc_data["Day_No"], pr_auc_data["Test_PR_AUC"], "o-", color="orange"
        )
