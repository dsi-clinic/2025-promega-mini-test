...
#SBATCH --output=combined_classifier/logs/multimodal_%A_%a.out
#SBATCH --error=combined_classifier/logs/multimodal_%A_%a.err
...
cd $PROJ_ROOT
mkdir -p combined_classifier/logs
...
OUTDIR="combined_classifier/outputs_multimodal/${BACKBONE}_${INPUT}_${FUSION}"
...
python combined_classifier/train_multimodal.py \
    --backbone "$BACKBONE" \
    --input-mode "$INPUT" \
