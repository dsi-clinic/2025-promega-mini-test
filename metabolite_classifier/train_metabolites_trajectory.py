#!/usr/bin/env python3
"""
Metabolite Organoid Quality Classification — Cumulative Trajectory Version

Trains per-day classifiers using LightGBM where each day's model uses
ALL metabolite measurements from day 1 through that day, flattened into
a single feature vector.  This means:

    Dy03 → features from [Dy03]                       (~10 cols)
    Dy06 → features from [Dy03, Dy06]                 (~20 cols)
    Dy10 → features from [Dy03, Dy06, Dy10]           (~30 cols)
    ...
    Dy30 → features from [Dy03 … Dy30]                (~80+ cols)

Each row is one organoid.  Label is the final Day-30 quality label.

THRESHOLD STRATEGY: Validation-based (same as CPU script)
...
CLASS IMBALANCE: Matches CPU script options (class_weight / scale_pos_weight / both)
METRICS: Internal optimization uses pos_label="Not Acceptable" (1)
         Reporting/Output uses pos_label="Acceptable" (1) for legacy format

Usage:
    python3 train_metabolites_trajectory.py
    python3 train_metabolites_trajectory.py --imbalance scale_pos_weight --scoring recall_notacceptable
    python3 train_metabolites_trajectory.py --imbalance both --scoring f1_notacceptable
    python3 train_metabolites_trajectory.py --growth-order 2  # add acceleration features
"""
...
def compute_growth_features(df):
    """Add first-order growth features (difference between consecutive timepoints)."""
    df = df.copy()
...
# ---------------------------------------------------------------------
def build_trajectory_dataset(df, target_day, all_days_sorted, feature_cols):
    """
    Build a cumulative trajectory dataset for a given target day.

    For each organoid, flattens all available days <= target_day into a
    single row with columns like Glucose_Dy03, Glucose_Dy06, etc.

    Args:
        df: Long-format DataFrame (one row per organoid per day)
        target_day: Day number to predict at (includes all days up to this)
        all_days_sorted: Sorted list of all day numbers in the dataset
        feature_cols: List of feature column names to flatten

    Returns:
        X: Features DataFrame (one row per organoid)
        y: Labels Series (string)
        groups: Groups Series (organoid IDs)
        ids: Organoid IDs Series
    """
    df = df.copy()
    if df.empty:
        return (
    ...
    allowed_days = [d for d in all_days_sorted if d <= target_day]

    df = df[df["day"].isin(allowed_days)].copy()
    if df.empty:
        return (
            pd.DataFrame(),
            pd.Series(dtype=object),
    ...
    for org_id in ids:
        ...
        row = {"ID": org_id}
        # Label is final organoid label, same across all days
        ...
        for d in allowed_days:
            ...
            if df_id_d.empty:
                for col in feature_cols:
                    row[f"{col}_Dy{d:02d}"] = np.nan
            else:
                r = df_id_d.iloc[0]
                for col in feature_cols:
                    row[f"{col}_Dy{d:02d}"] = r.get(col, np.nan)
...
# ---------------------------------------------------------------------
# Cleaning (no scaling — LightGBM is scale-invariant)
# ---------------------------------------------------------------------
def clean_data(X_train, X_val=None, X_test=None):
    ...
    # Fill NaNs (missing timepoints for some organoids)
...
# ---------------------------------------------------------------------
# Scoring, thresholding, model building (same as CPU)
# ---------------------------------------------------------------------
...
def tune_threshold_on_val(scores_acc, labels_str, scoring):
    ...
    for t in thresholds:
        ...
        if scoring == "f1_weighted":
            score = f1_score(
                labels_bin, y_pred_bin, average="weighted", zero_division=0
            )
        elif scoring == "f1_notacceptable":
...
def build_model(imbalance, y_train, print_weights=False):
    ...
    if len(classes) < 2:
        ...
        if print_weights and imbalance in ["scale_pos_weight", "both"]:
            print(
                f"    scale_pos_weight = {scale_pos_weight_val:.2f} (NA={n_pos}, Acc={n_neg})"
            )
...
def save_organoid_predictions(ids, y_test_arr, y_pred, y_score_na, output_path):
    """
    ...
    for idx in range(len(ids)):
        org_id = ids.iloc[idx] if hasattr(ids, "iloc") else ids[idx]
        true_label_str = y_test_arr[idx]
...
# ---------------------------------------------------------------------
# Core training loop (per-day, cumulative trajectory)
# ---------------------------------------------------------------------
def train_trajectory_classifier_per_day(
    train_df,
    val_df,
    test_df,
    output_dir,
    ...
    """Train cumulative trajectory classifier for each day.

    Mirrors the CPU per-day script structure:
    ...
    results_summary = []
    unique_days = sorted(all_days_sorted)
    per_day_info = {}
    ...
    # to help with the wider feature space at later days.
    param_grid = {
        "max_depth": [3, 6],
        "num_leaves": [31, 47, 63],
    ...
    # ========== PHASE 1: GridSearchCV on train ==========
    print("[PHASE 1] Hyperparameter tuning on TRAIN data (trajectory)")

    for target_day in unique_days:
        day_label = f"Dy{target_day:02d}"

        # Build cumulative trajectory dataset for this day
        X_train, y_train, groups_train, _ = build_trajectory_dataset(
            train_df, target_day, all_days_sorted, feature_cols
        )

        if X_train.empty or len(X_train) == 0:
            print(f"  {day_label}: no training data, skipping.")
            continue

        n_acc = (np.asarray(y_train) == TRAIN_LABEL_NEG).sum()
        n_na = (np.asarray(y_train) == TRAIN_LABEL_POS).sum()
        n_features = X_train.shape[1]

        print(
            f"\n  [{day_label}] Train n={len(X_train)} (Acc={n_acc}, NA={n_na}), features={n_features}"
        )

        X_train_clean, _, _ = clean_data(X_train)

        if X_train_clean.shape[1] == 0:
            print(f"    No features left after cleaning; skipping {day_label}.")
            continue

        if len(np.unique(y_train)) < 2:
            print(f"    Only one class in training data; skipping {day_label}.")
            continue
    ...
    for target_day in unique_days:
        if target_day not in per_day_info:
            continue

        day_label = per_day_info[target_day]["day_label"]

        X_train, y_train, _, _ = build_trajectory_dataset(
            train_df, target_day, all_days_sorted, feature_cols
        )
        X_val, y_val, _, _ = build_trajectory_dataset(
            val_df, target_day, all_days_sorted, feature_cols
        )

        if X_val.empty or len(X_val) == 0:
            print(f"  {day_label}: no val data, using default threshold=0.5")
            thresholds_per_day[target_day] = 0.5
            per_day_info[target_day]["threshold"] = 0.5
            continue

        X_train_clean, X_val_clean, _ = clean_data(X_train, X_val=X_val)

        model = build_model(imbalance, y_train)
        model.set_params(**per_day_info[target_day]["best_params"])
        ...
        y_val_arr = (
            y_val.to_numpy() if hasattr(y_val, "to_numpy") else np.asarray(y_val)
        )

        if len(np.unique(y_val_arr)) > 1:
            threshold, thresh_score = tune_threshold_on_val(
                scores_acc, y_val_arr, scoring
            )
            print(
                f"  {day_label}: threshold={threshold:.3f}, {scoring}={thresh_score:.3f}"
            )
            ...
            threshold = 0.5
            print(f"  {day_label}: single-class val, using threshold=0.5")

        thresholds_per_day[target_day] = threshold
        per_day_info[target_day]["threshold"] = threshold
    ...
    for target_day in unique_days:
        ...
        info = per_day_info[target_day]
        day_label = info["day_label"]
        threshold = info.get("threshold", 0.5)

        X_trainval, y_trainval, _, _ = build_trajectory_dataset(
            trainval_df, target_day, all_days_sorted, feature_cols
        )
        X_test, y_test, _, ids_test = build_trajectory_dataset(
            test_df, target_day, all_days_sorted, feature_cols
        )

        if X_test.empty or len(X_test) == 0:
            print(f"  {day_label}: no test data, skipping.")
            continue

        print(
            f"\n  [{day_label}] Trainval n={len(X_trainval)}, Test n={len(X_test)}, features={X_trainval.shape[1]}"
        )

        X_trainval_clean, _, X_test_clean = clean_data(X_trainval, X_test=X_test)

        if X_trainval_clean.shape[1] == 0:
            print(f"    No features; skipping {day_label}.")
            continue
        ...
        plt.title(
            f"Top {top_n} Feature Importances - {day_label} (trajectory)",
            fontweight="bold",
            fontsize=14,
        )
        ...
        y_pred_test = np.where(
            y_score_acc >= threshold, REPORT_LABEL_POS, REPORT_LABEL_NEG
        )
        ...
        report = classification_report(
            y_test_arr, y_pred_test, output_dict=True, zero_division=0
        )
        ...
        cm = confusion_matrix(
            y_test_arr, y_pred_test, labels=[REPORT_LABEL_POS, REPORT_LABEL_NEG]
        )
        ...
        print(
            f"    Recall_NA={recall_notaccept:.3f}, Prec_NA={precision_notaccept:.3f}, "
        ...
        # Save metrics JSON (matches CPU format)
        day_num = int(target_day)
        allowed_days = [d for d in all_days_sorted if d <= target_day]
        metrics = {
            "day": day_label,
            "day_no": day_num,
            "trajectory_days_included": [f"Dy{d:02d}" for d in allowed_days],
            "n_features": int(X_trainval_clean.shape[1]),
        ...
        plt.title(f"Confusion Matrix - {day_label}")
        plt.colorbar()
    ...
    print(
        summary_df[
            [
                "Day",
                "N_Features",
                "Test_F1_NotAcceptable",
                "Test_Recall_NotAcceptable",
                "Test_Specificity",
                "TP",
                "FP",
                "FN",
    ...
    for target_day in unique_days:
        if (
            target_day in per_day_info
            and "feature_importance_df" in per_day_info[target_day]
        ):
            feat_df = per_day_info[target_day]["feature_importance_df"]
    ...
    for feature in all_features:
        ...
        for target_day in unique_days:
            if (
                target_day not in per_day_info
                or "feature_importance_df" not in per_day_info[target_day]
            ):
                continue
            feat_df = per_day_info[target_day]["feature_importance_df"]
            ...
            if len(feature_row) > 0:
                ...
                row[f"Day_{target_day}_importance"] = importance
                importance_values.append(importance)
            else:
                row[f"Day_{target_day}_importance"] = 0.0
    ...
    if feature_summary_data:
        ...
        feature_summary_df.to_csv(
            model_dir / "feature_importance_summary.csv", index=False
        )

        top_features = feature_summary_df.head(15)
        day_cols = [
            c
            for c in feature_summary_df.columns
            if c.startswith("Day_") and c.endswith("_importance")
        ]
        ...
        if len(day_cols) > 0:
            ...
            ax.set_title(
                "Top 15 Features: Importance Across Days",
                fontweight="bold",
                fontsize=14,
            )
            ax.set_xticks(x)
            ...
            plt.tight_layout()
            plt.savefig(
                model_dir / "feature_importance_comparison.png",
                dpi=150,
                bbox_inches="tight",
            )
            plt.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Cumulative Trajectory Metabolite Classifiers (CPU)"
    )
    parser.add_argument(
        "--imbalance",
    ...
    output_dir = "analysis/metabolites/LightGBM/outputs_metabolites_trajectory"
    ...
    # Determine which columns are features to flatten
    ...
    metadata_cols = {"ID", "batch", "label", "DY", "img_path", "mask_path", "day"}
    # Drop initial_concentration columns (same as CPU cols_to_drop_base)
    drop_suffixes = {"_initial_concentration"}
    feature_cols = [
        c
        for c in df_all.columns
        if c not in metadata_cols
        and df_all[c].dtype != "O"
        and not any(c.endswith(s) for s in drop_suffixes)
    ]

    print(f"\nAll days: {[f'Dy{d:02d}' for d in all_days_sorted]}")
    print(f"Feature columns per day: {len(feature_cols)}")
    print(f"Features: {feature_cols}\n")

    growth_suffix = f"_g{args.growth_order}"
    model_name = f"lgbm_trajectory_{args.imbalance}_{args.scoring}{growth_suffix}"

    train_trajectory_classifier_per_day(
        train_df,
        val_df,
        test_df,
        output_dir,
