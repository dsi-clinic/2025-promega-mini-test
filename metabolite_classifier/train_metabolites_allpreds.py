...
This version also writes full-dataset organoid flags (fp/fn/accurate)
for specific days without overwriting existing outputs.

Usage:
    python train_metabolites_allpreds.py
    python train_metabolites_allpreds.py --cv_scoring f1_notaccept --threshold_metric f1_notaccept
    python train_metabolites_allpreds.py --cv_scoring macro_f1 --threshold_metric macro_f1
...
def build_full_df_from_all_data(all_data_json):
    """Build a metabolite dataframe from all_data.json with survey labels."""
    label_by_organoid = {}
    for key, record in all_data_json.items():
        day_id = record.get("dayID")
        if day_id != "Dy30":
            continue
        survey = record.get("survey") or {}
        evaluations = survey.get("evaluations", [])
        label = compute_majority_label(evaluations)
        if label:
            org_id = extract_organoid_id(key)
            label_by_organoid[org_id] = label
    ...
    for key, record in all_data_json.items():
        ...
        if not day_id:
            continue
        org_id = extract_organoid_id(key)
        label = label_by_organoid.get(org_id)
        if not label:
            continue
        ...
        row = {
            "ID": org_id,
            "batch": get_batch_prefix(record.get("BA")),
            "label": label,
            "DY": day_id,
        ...
        for met_name, met_info in metabolites.items():
            if "concentration_uM" in met_info:
                row[f"{met_name}_concentration_uM"] = met_info["concentration_uM"]
            if "initial_concentration" in met_info:
                row[f"{met_name}_initial_concentration"] = met_info[
...
def save_organoid_predictions(selected_test_df, y_test, y_pred, y_score, output_path):
    ...
    Args:
        selected_test_df: Test DataFrame with organoid IDs and labels.
        y_test: True labels (string format: "Acceptable" or "Not Acceptable").
        y_pred: Predicted labels (string format).
...
def save_organoid_flags_json(selected_df, y_true, y_pred, y_score, output_path):
    """Save per-organoid flags and predictions as JSON.

    Args:
        selected_df: DataFrame with organoid IDs.
        y_true: True labels (string format).
        y_pred: Predicted labels (string format).
    ...
    for idx in range(len(selected_df)):
        org_id = selected_df.iloc[idx]["ID"]
        true_label = label_map.get(y_true[idx], 0)
        pred_label = label_map.get(y_pred[idx], 0)
        pred_prob = float(y_score[idx])
        accurate = int(pred_label == true_label)
    ...
    print(f"  Saved organoid flags to {output_path}")


def prepare_data_for_day(df, day_num, cols_to_drop_base):
    """Prepare features, labels, and groups for a specific day.

    Applies day-specific filtering (e.g., drops Malate for days <= 10,
    drops growth features for day 3).

    Args:
        df: DataFrame with metabolite features and labels.
        day_num: Day number (integer, e.g., 3, 13, 28).
        cols_to_drop_base: Base list of columns to drop.

    Returns:
        Tuple of (X, y, groups) where X is feature DataFrame, y is labels Series,
        and groups is organoid ID Series for group-based cross-validation.
    """
    df_day = df.copy()
...
def clean_and_scale_data(X_train, X_val=None, X_test=None):
    ...
    Drops all-NaN columns, constant columns, near-constant columns, fills remaining NaNs,
    and applies StandardScaler transformation. Same transformations applied to val/test.
...
def clean_data_no_scaling(X_train, X_val=None, X_test=None):
    ...
    Drops all-NaN columns, constant columns, near-constant columns, and fills remaining NaNs.
    Does not apply scaling transformation.
    ...
    Returns:
        Tuple of (X_train, X_val, X_test) cleaned DataFrames.
    """
    # Drop all-NaN columns
    all_nan_cols = X_train.columns[X_train.isna().all()].tolist()
    if all_nan_cols:
...
def save_calibration_diagnostic(model_dir, val_scores_all, y_val_bin_all):
    ...
    Creates calibration bins and reliability curve plot based on validation predictions.

    Args:
        model_dir: Directory path to save calibration outputs.
        val_scores_all: Array of validation probability scores (pooled across all days).
        y_val_bin_all: Array of binary validation labels (1=Acceptable, 0=Not Acceptable).
    """
...
def train_metabolite_classifier_per_day(
    ...
    for days in unique_days:
        ...
        if len(classes_comb) < 2:
            ...
            weight_dict_comb = {
                cls: float(w) for cls, w in zip(classes_comb, weights_comb)
            }
        ...
        # Full-dataset organoid flags for selected days
        if days in ALL_ORGANOID_DAYS:
            day_full = full_df[full_df["DY"] == days].copy()
            if len(day_full) == 0:
                print(f"  No full-dataset rows found for {days}")
            else:
                X_full, y_full, _ = prepare_data_for_day(
                    day_full, day_num, cols_to_drop_base
                )
                X_full = X_full.reindex(columns=X_combined.columns, fill_value=0)
                if use_scaling:
                    _, _, X_full_scaled, _ = clean_and_scale_data(
                        X_combined, X_test=X_full
                    )
                else:
                    _, _, X_full_scaled = clean_data_no_scaling(
                        X_combined, X_test=X_full
                    )

                proba_full = final_model.predict_proba(X_full_scaled)
                if "Acceptable" in classes_order:
                    acc_idx = classes_order.index("Acceptable")
                    y_score_full = proba_full[:, acc_idx]
                else:
                    y_score_full = proba_full[:, 1]

                y_pred_full = np.where(
                    y_score_full >= threshold_used, "Acceptable", "Not Acceptable"
                )
                save_organoid_flags_json(
                    day_full.reset_index(drop=True),
                    y_full.to_numpy(),
                    y_pred_full,
                    y_score_full,
                    day_dir / "organoid_flags_all.json",
                )
    ...
    for feature in all_features:
        ...
        for days in unique_days:
            if (
                days not in per_day_info
                or "feature_importance_df" not in per_day_info[days]
            ):
                continue
    ...
    if feature_summary_data:
        ...
        top_10 = feature_summary_df.head(10)[
            ["feature", "avg_importance", "num_days_used"]
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train Metabolite Classifiers (Full Predictions for Dy28/Dy30)"
    )
    parser.add_argument(
        "--cv_scoring",
    ...
    full_data_path = "all_data.json"
    output_dir = "analysis/metabolites/classifier/outputs_metabolites_allpreds"
    ...
    full_df = build_full_df_from_all_data(full_data_json)
    ...
    full_df = compute_growth_features(full_df)

    # Construct model name based on args
    model_name = (
        f"lgbm_per_day_noscale_cv_{args.cv_scoring}_thresh_{args.threshold_metric}"
    )
