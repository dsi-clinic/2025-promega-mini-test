#!/usr/bin/env python3
"""
Metabolite Organoid Quality Classification (Logistic Regression).
Same pipeline/output structure as LightGBM version, but model is LogisticRegression.
"""
...
def train_metabolite_classifier_per_day(
    ...
    print(f"\n{'=' * 60}")
    print("Training Metabolite Classifier (LOGREG)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train Metabolite Classifiers (Logistic Regression)"
    )
    parser.add_argument(
        "--cv_scoring",
    ...
    model_name = (
        f"logreg_per_day_noscale_cv_{args.cv_scoring}_thresh_{args.threshold_metric}"
    )
