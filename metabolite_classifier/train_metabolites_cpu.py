...
def save_organoid_predictions(
    selected_test_df, y_test, y_pred, y_score_na, output_path
):
    """
...
def prepare_data_for_day(df, day_num, cols_to_drop_base):
    ...
    if day_num == 3:
        cols_to_drop.extend(
            [g for g in growth_features + accel_features if g in df_day.columns]
        )
    elif day_num <= 6:
...
def train_metabolite_classifier_per_day(
    ...
    for days in unique_days:
        ...
        X_train, y_train, _ = prepare_data_for_day(
            day_train, day_num, cols_to_drop_base
        )
