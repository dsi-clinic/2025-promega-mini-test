            ...
            for _, row in df.iterrows():
                ...
                for col in row.index:
                    ...
                    if pd.notna(val) and isinstance(val, str) and (
                        ...
                        if image_id:
                            image_id_core = re.sub(r"\b(INV|PRE|POST|STITCH|STCH|Z\d+)\b", "", image_id, flags=re.IGNORECASE)
                            image_id_core = re.sub(r"\s+", " ", image_id_core).strip()  # normalize whitespace
                            image_id_clean = clean_id_for_json(image_id_core)
                        else:
                            image_id_clean = None
                        ...
                        if parsed_meta == {}:
                            print(f" Could not parse image_id: {image_id} from {organoid_id} in {basename}")
