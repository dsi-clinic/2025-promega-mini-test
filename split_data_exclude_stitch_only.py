...
EXCLUDE ONLY STITCHED SAMPLES (Keep split organoids)

This variant excludes ONLY stitched data to isolate the effect of stitching.
Split organoids are KEPT in this dataset.

Output files:
- both_train_exclude_stitch_only.json
- both_val_exclude_stitch_only.json
- both_test_exclude_stitch_only.json
"""
import json
import argparse
import numpy as np
...
# ============================================================
# HELPER FUNCTIONS (copied from split_data_no_stitch.py)
# ============================================================
...
def is_stitched_only(common_key, img_path=None):
    """
    Check if sample is STITCHED.
    EXCLUDE: stitched-only images AND split+stitched images
    KEEP: split-only images and nosplit_nostitch images
    """
    ...
    # Exclude if has stitched (whether or not it also has split)
    # This excludes: stitched-only AND split+stitched
    # This keeps: split-only (no stitched) and nosplit_nostitch
    return has_stitched
...
def collect_organoid_data(all_data, batches=['BA1', 'BA2'], require_metabolites=True):
    ...
    EXCLUDING ONLY stitched samples (keeping split samples).
    """
    ...
    for key, value in all_data.items():
        ...
        timepoint_data = {
            ...
            'day': day,
            'is_stitched': is_stitched_only(common_key, img_path)  # Flag for later filtering
        }
    ...
    # Third pass: EXCLUDE ENTIRE ORGANOIDS if ANY day has stitched
    ...
    for organoid_id, org_info in organoid_data_raw.items():
        # Check if ANY timepoint has stitched
        has_bad_timepoint = False
        for day, day_data in org_info['timepoints'].items():
            if day_data.get('is_stitched', False):
        ...
        for day, day_data in org_info['timepoints'].items():
            clean_timepoint = {k: v for k, v in day_data.items() if k != 'is_stitched'}
            organoid_data[organoid_id]['timepoints'][day] = clean_timepoint
    
    print(f"  Excluded {excluded_organoids} entire organoids (had stitched timepoints)")
    print(f"  Excluded {excluded_stitched} stitched timepoints from excluded organoids")
    print(f"  Kept {len(organoid_data)} pure organoids (no stitched, but may have split)")
...
def main():
    ...
    print("="*80)
    print("EXCLUDE STITCH ONLY - Keep Split Organoids")
    print("="*80)
    ...
    print("   EXCLUDING: Stitched samples ONLY (keeping split organoids)")
    ...
    train_file = output_dir / 'both_train_exclude_stitch_only.json'
    val_file = output_dir / 'both_val_exclude_stitch_only.json'
    test_file = output_dir / 'both_test_exclude_stitch_only.json'
    ...
    print("="*80)
    print("\nThese splits EXCLUDE stitched samples but KEEP split organoids.")
    print("This isolates the effect of stitching on model performance.")
