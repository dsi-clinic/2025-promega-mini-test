...
def run_base_mode(all_data):
    ...
    organoid_data = collect_organoid_data(
        all_data, batches=["BA1", "BA2"], require_metabolites=True
    )
    ...
    train_file, val_file, test_file = save_splits(
        train_data, val_data, test_data, "both", "base"
    )
    print(f"\n✓ Saved: {train_file}")
...
def run_switch1_mode(all_data, intersection_organoids):
    ...
    all_image_organoids = collect_organoid_data(
        all_data, batches=["BA1", "BA2"], require_metabolites=False
    )

    print(f"Total BA1+BA2 organoids with image: {len(all_image_organoids)}")
    print(
        f"Additional image-only organoids: {len(all_image_organoids) - len(intersection_organoids)}"
    )

    # Split by organoid: 80% training / 20% test, then split training into train/val
    train_data, val_data, test_data = split_by_organoid(
        all_image_organoids, random_seed=RANDOM_SEED
    )
    ...
    # Save
    train_file, val_file, test_file = save_splits(
        train_data, val_data, test_data, "image", "switch1"
    )
    print(f"\n✓ Saved: {train_file}")
    print(f"✓ Saved: {val_file}")
    print(f"✓ Saved: {test_file}")
...
def run_switch2_mode(all_data):
    ...
    organoid_data = collect_organoid_data(
        all_data, batches=["BA1", "BA2", "BA3", "BA4"], require_metabolites=True
    )
    ...
    # Save
    train_file, val_file, test_file = save_splits(
        train_data, val_data, test_data, "both", "switch2"
    )
    print(f"\n✓ Saved: {train_file}")
    print(f"✓ Saved: {val_file}")
    print(f"✓ Saved: {test_file}")
...
def run_switch3_mode(all_data):
    ...
    organoid_data = collect_organoid_data(
        all_data, batches=["BA1", "BA2", "BA3", "BA4"], require_metabolites=False
    )
    ...
    # Save
    train_file, val_file, test_file = save_splits(
        train_data, val_data, test_data, "image", "switch3"
    )
    print(f"\n✓ Saved: {train_file}")
    print(f"✓ Saved: {val_file}")
    print(f"✓ Saved: {test_file}")
