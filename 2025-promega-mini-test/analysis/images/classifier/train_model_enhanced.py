...
from analysis.images.preprocessing.stitched_preprocessing import preprocess_stitched_pil
...
DEFAULT_DATA_DIR = Path("analysis/images/classifier/data/preprocessed/512x384/majority/")
DEFAULT_OUT_ROOT = Path("analysis/images/classifier/outputs_512x384_two_level_aug")
BATCH_SIZE = 16
...
def get_transforms(split: str,
    ...
    if level == "geom_photo":
        ...
        return T.Compose(base_resize + geom + [
            T.ToTensor(),
...
class OrganoidDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = preprocess_stitched_pil(img, self.img_paths[base_idx])
...
def make_loader(imgs, labels, split, transform, batch_size, duplicate_hflip=False, shuffle=None):
    if shuffle is None:
        shuffle = (split == "train")
    ds = OrganoidDataset(imgs, labels, split=split, transform=transform, duplicate_hflip=duplicate_hflip)
...
def run_training_for_day(day_json_path: Path, backbone_key: str, backbone_name: str,
    ...
    # Transforms
    t_train = get_transforms(split="train", level=augment_level,
                             mean=norm_mean, std=norm_std,
                             geom_cfg=geom_cfg, photo_cfg=photo_cfg)
    t_eval  = get_transforms(split="val",   level="none",
                             mean=norm_mean, std=norm_std)

    # Loaders
    train_loader = make_loader(X_tr, y_tr, split="train", transform=t_train,
                               batch_size=train_bs, duplicate_hflip=duplicate_hflip_train, shuffle=True)
    val_loader   = make_loader(X_val, y_val, split="val",   transform=t_eval,
                               batch_size=val_bs, duplicate_hflip=False, shuffle=False)
    test_loader  = make_loader(X_test, y_test, split="test", transform=t_eval,
                               batch_size=val_bs, duplicate_hflip=False, shuffle=False)
    ...
    plt.tight_layout(); plt.savefig(model_dir / "training_curves.png"); plt.close()
    ...
    with (model_dir / "metrics_val.json").open("w") as f:
        json.dump({"day": day_json_path.stem, "split": "val", "accuracy": float(val_acc), "f1": float(val_f1),
                   "n": int(len(y_val)), "batch_size": int(val_bs)}, f, indent=2)
...
def main():
    ...
    # Directories
    parser.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR, help="Directory with per-day JSONs (Dy*.json)")
    parser.add_argument("--out-root", type=Path, default=DEFAULT_OUT_ROOT, help="Root directory to save outputs")
    parser.add_argument("--exp-name", type=str, default=None, help="Experiment name (subfolder under out-root). If omitted, a timestamped name is used.")
    ...
    # Augment control
    parser.add_argument("--augment-level", choices=["none","geom","geom_photo"], default="geom",
                        help="Augmentation level for TRAIN split")
    parser.add_argument("--duplicate-train-hflip", action="store_true",
                        help="Duplicate each training sample with a horizontally flipped copy (keep both).")

    # Optional magnitudes
    parser.add_argument("--rot-deg", type=float, default=15.0, help="Geom: RandomRotation degrees")
    parser.add_argument("--rrc-min-scale", type=float, default=0.9, help="Geom: RandomResizedCrop min scale")
    parser.add_argument("--rrc-max-scale", type=float, default=1.1, help="Geom: RandomResizedCrop max scale")
    parser.add_argument("--affine-deg", type=float, default=5.0, help="Geom: RandomAffine degrees")
    parser.add_argument("--affine-trans", type=float, default=0.02, help="Geom: RandomAffine translate fraction")
    parser.add_argument("--affine-min-scale", type=float, default=0.98, help="Geom: RandomAffine min scale")
    parser.add_argument("--affine-max-scale", type=float, default=1.02, help="Geom: RandomAffine max scale")

    parser.add_argument("--photo-noise-std", type=float, default=0.01, help="Photo: Gaussian noise std")

    # Normalization
    parser.add_argument("--norm-mean", type=float, nargs=3, default=[0.5,0.5,0.5], help="Normalization mean (RGB)")
    parser.add_argument("--norm-std",  type=float, nargs=3, default=[0.5,0.5,0.5], help="Normalization std (RGB)")
    ...
    print(f"🧩 Augment level: {args.augment_level} | Duplicate HFlip: {args.duplicate_train_hflip}")
    ...
    per_day_best = {}
    for json_file in sorted(args.data_dir.glob("Dy*.json"), key=lambda p: day_to_int(p.stem)):
        day = json_file.stem
        best = None
