...
class SingleDayOrganoidDataset(Dataset):
    ...
    """
    def __init__(self, organoid_ids, series_metadata, data, target_day, transform=None, image_key='img_path', use_rgb_mask=False):
        self.samples = []
        ...
        for org_id in organoid_ids:
            ...
            if img_path is None or not Path(img_path).exists():
                continue
            if image_key == 'overlay_path' and overlay_path and not Path(overlay_path).exists():
                overlay_path = ''
        ...
        print(f"  Loaded {len(self.samples)} samples for day ~{target_day} (image_key={image_key}, use_rgb_mask={use_rgb_mask})")
    ...
    def __getitem__(self, idx):
        ...
        if C == 3:
            ...
            mean = torch.tensor([0.485, 0.456, 0.406, 0.5], dtype=torch.float32).view(4, 1, 1)
            std = torch.tensor([0.229, 0.224, 0.225, 0.5], dtype=torch.float32).view(4, 1, 1)
        img = (img - mean) / std
...
def train_for_day(target_day, train_ids, val_ids, test_ids, 
                  ...
                  image_key='img_path', use_rgb_mask=False, in_channels=3, save_model=True, seed=None):
