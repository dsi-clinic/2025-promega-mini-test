...
class OrganoidCNN_TChange(nn.Module):
    ...
    def forward(self, x, days_norm):
        ...
        feat_var = ((feats - feat_mean.unsqueeze(1)) ** 2 * mask_3d).sum(dim=1) / n_real.unsqueeze(1)  # (B, d_cnn)
        ...
        last_feat = feats[torch.arange(B, device=feats.device), last_indices]  # (B, d_cnn)
        ...
        combined = torch.cat([pooled, feat_var, delta_first_last, mean_diffs], dim=-1)  # (B, 4*d_cnn)
