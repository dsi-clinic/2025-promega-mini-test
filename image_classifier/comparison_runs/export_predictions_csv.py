...
def export_predictions(base_dir, output_dir=None, model_filter=None):
    ...
    if model_filter is not None:
        models_with_predictions = [
            m for m in models_with_predictions if m in model_filter
        ]
    ...
    for model_type in models_with_predictions:
        ...
        for day_dir in sorted(
            model_path.iterdir(),
            key=lambda p: (p.name.replace("day_", "").replace("_", "."),),
        ):
            ...
            with open(out_path, "w", newline="") as f:
                w = csv.DictWriter(
                    f, fieldnames=PREDICTION_COLUMNS, extrasaction="ignore"
                )
                w.writeheader()
                w.writerows(rows)
...
def main():
    p = argparse.ArgumentParser(
        description="Export per-organoid predictions to CSVs (organoid_id, split, day, y_true, y_pred, y_score)"
    )
    p.add_argument(
        "--base_dir",
        "-b",
        required=True,
        help="Path to per_day_study_overlay dir (e.g. regeneration/run_5_all_samples/per_day_study_overlay)",
    )
    p.add_argument(
        "-o",
        "--output_dir",
        default=None,
        help="Output directory for CSVs (default: base_dir)",
    )
    p.add_argument(
        "--model",
        default=None,
        choices=["per_day", "effnet_ts", "effnet_tchange"],
        help="Export only this model (default: all that have predictions)",
    )
    args = p.parse_args()
    model_filter = [args.model] if args.model else None
    written = export_predictions(
        args.base_dir, output_dir=args.output_dir, model_filter=model_filter
    )
    if not written:
        print(
            "No prediction data found (results.json must contain train/val/test_predictions)."
        )
        return
