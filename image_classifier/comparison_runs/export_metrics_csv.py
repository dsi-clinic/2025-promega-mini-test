...
def collect_rows(
    include_kfold=False, setup_filter=None, overlay_base=None, splits=None
):
...
def main():
    ...
    p = argparse.ArgumentParser(
        description="Export per-day study results to CSV (columns: setup, fold, model, day, threshold, accuracy, ..., balanced_acc, ..., optimal_threshold)"
    )
    p.add_argument(
        "-o",
        "--output",
        default=None,
        help="Output CSV path (default: comparison_runs/metrics.csv)",
    )
    p.add_argument(
        "--kfold",
        action="store_true",
        help="Also include per_day_study_kfold if present",
    )
    p.add_argument(
        "--setup",
        default=None,
        choices=["rgb", "overlay", "rgb_mask"],
        help="Only export this setup (e.g. overlay)",
    )
    p.add_argument(
        "--overlay_dir",
        default=None,
        help="If set, read overlay results from this dir (for parallel regeneration runs)",
    )
    p.add_argument(
        "--split",
        default="test",
