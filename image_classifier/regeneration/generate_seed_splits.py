...
def main():
    p = argparse.ArgumentParser(
        description="Generate train/val/test splits for a given seed"
    )
    p.add_argument(
        "--seed", type=int, required=True, help="Random seed for split generation"
    )
    p.add_argument(
        "--output_dir",
        type=str,
    ...
    all_data_path = REPO_ROOT / ALL_DATA_JSON
    ...
    print(
        f"Collected {len(organoid_data)} organoids (BA1+BA2, both image & metabolite)"
    )

    train_data, val_data, test_data = split_by_organoid(
        organoid_data, random_seed=args.seed
    )
