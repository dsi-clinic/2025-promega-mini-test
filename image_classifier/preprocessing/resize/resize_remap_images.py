...
def create_mapping(
    entries: dict, base_folder: str, batch_id: str, day_num: int, overwrite: bool
):
    ...
    for img_id, img_path_str, orig_um_px in matches:
        ...
        main_id = verification.get(
            "main_id", img_id.replace(" ", "_")
        )  # fallback to img_id if missing
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batches", required=True, help="Comma-separated batch numbers, e.g. 1,2,3"
    )
    parser.add_argument(
        "--days", required=True, help="Comma-separated day numbers, e.g. 3,6,8"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
