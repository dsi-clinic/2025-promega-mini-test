...
def main():
    ap = argparse.ArgumentParser(
        description="Plate overlays from MT-mask JSON (saves *_MT.png)."
    )
    ap.add_argument(
        "--json",
        required=True,
        help="Path to JSON with 'Best Z Filename' and 'MT Mask Path'.",
    )
    ap.add_argument(
        "--outdir",
    ...
    for (BA, day), wells in groups.items():
        ...
        for r_i, row in enumerate(ROWS):
            for c_i, col in enumerate(COLS):
                ...
                if img.shape[:2] != (tile_h, tile_w):
                    img = cv2.resize(
                        img, (tile_w, tile_h), interpolation=cv2.INTER_LINEAR
                    )
        ...
        out_png = (
            outdir / f"{sanitize(BA)}_{sanitize(day)}_plate_overlay{args.suffix}.png"
        )
