...
def draw_outline_overlay(
    img_bgr: np.ndarray, mask_bin: np.ndarray, color=(0, 255, 0), thickness=2
) -> np.ndarray:
...
def process_mapping_json(
    ...
    for k, rec in mapping.items():
        ...
        if miss_img or miss_msk:
            missing += 1
            if miss_img:
                reason_counts["missing_img_file"] += 1
            if miss_msk:
                reason_counts["missing_mask_file"] += 1
            reason = "missing file(s): "
            if miss_img:
                reason += f"[img:{img_p}] "
            if miss_msk:
                reason += f"[mask:{mask_p}] "
        ...
        if img is None or mask_bin is None:
            missing += 1
            if img is None:
                reason_counts["decode_img"] += 1
            if mask_bin is None:
                reason_counts["decode_mask"] += 1
            missing_pairs.append(
                (k, f"decode failed: img({img is None}), mask({mask_bin is None})")
            )
            continue
        ...
        if out_path.exists() and not overwrite:
            ...
            overlay = draw_outline_overlay(
                img, mask_bin, color=(0, 255, 0), thickness=2
            )
...
def main(overwrite=False, strict=False):
    ...
    for mp in tqdm(mapping_files, desc="Overlays"):
        ...
        if stats["missing_pairs"]:
            any_missing_samples.extend(
                [(stats["file"], *x) for x in stats["missing_pairs"]]
            )
        if stats["overlays_missing"]:
            any_overlay_missing_samples.extend(
                [(stats["file"], *x) for x in stats["overlays_missing"]]
            )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--strict",
        action="store_true",
        help="Assert overlays_on_disk == valid (img+mask) pairs.",
    )
    args = ap.parse_args()
