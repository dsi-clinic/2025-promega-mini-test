...
def safe_read_mask(path, target_hw=None):
    ...
    if target_hw is not None and (
        mk.shape[0] != target_hw[0] or mk.shape[1] != target_hw[1]
    ):
        mk = cv2.resize(
            mk, (target_hw[1], target_hw[0]), interpolation=cv2.INTER_NEAREST
        )
    return mk
...
def main():
    ap = argparse.ArgumentParser(
        description="Make plate mosaics with mask-outline overlays."
    )
    ...
    ap.add_argument(
        "--outdir",
        default="/net/projects2/promega/data-analysis/npy_outputs",
        help="Output directory for mosaics (PNG).",
    )
    ap.add_argument(
        "--outline-thickness", type=int, default=1, help="Contour outline thickness."
    )
    ...
    for (BA, day), wells in groups.items():
        ...
        for r_idx, row in enumerate(ROWS):
            for c_idx, col in enumerate(COLS):
                ...
                tile = (
                    safe_read_image(img_path)
                    if img_path and Path(img_path).exists()
                    else make_blank_tile()
                )
                if tile is None:
                ...
                if tile.shape[0] != tile_h or tile.shape[1] != tile_w:
                    tile = cv2.resize(
                        tile, (tile_w, tile_h), interpolation=cv2.INTER_LINEAR
                    )
                ...
                if mask_path and Path(mask_path).exists():
                    ...
                    if mk is not None:
                        tile = draw_mask_outline_on_image(
                            tile,
                            mk,
                            color=(255, 0, 255),
