...
def analyze_target_dimension_distribution(data, target_um_per_px):
    ...
    print(
        f"  Width  - min: {widths.min():4d}, median: {int(np.median(widths)):4d}, max: {widths.max():4d}"
    )
    print(
        f"  Height - min: {heights.min():4d}, median: {int(np.median(heights)):4d}, max: {heights.max():4d}"
    )
    print(
        f"  95th percentile - width: {int(np.percentile(widths, 95)):4d}, height: {int(np.percentile(heights, 95)):4d}"
    )
...
def process_entry(
    ...
    try:
        ...
        if mask_path:
            ...
            if mask is not None and mask.size > 0:
                ...
                if save_debug and stats["processed"] < 5:
                    ...
                    # Save raw image
                    imsave(
                        str(debug_dir / f"{main_id}_1_raw.png"),
                        (raw_image / raw_image.max() * 255).astype(np.uint8)
                    ...
                    imsave(
                        str(debug_dir / f"{main_id}_2_mask_stretched.png"),
                        mask,
                        check_contrast=False,
                    )
                    # Save unstretched mask
                    imsave(
                        str(debug_dir / f"{main_id}_3_mask_unstretched.png"),
                        mask_resized,
                        check_contrast=False,
                    )
                    # Save overlay
                    if len(raw_image.shape) == 3:
                        ...
                        imsave(
                            str(debug_dir / f"{main_id}_4_overlay.png"),
                            overlay,
                            check_contrast=False,
                        )
        ...
        image_processed = resize_and_pad(
            raw_image, target_width, target_height, TARGET_SIZE, is_mask=False
        )

        # Resize and pad mask (black padding for background label)
        mask_processed = None
        if mask_resized is not None:
            mask_processed = resize_and_pad(
                mask_resized, target_width, target_height, TARGET_SIZE, is_mask=True
            )
        ...
        if save_debug and stats["processed"] < 5 and mask_processed is not None:
            debug_dir = output_images_dir.parent / "debug"
            imsave(
                str(debug_dir / f"{main_id}_5_final_image.png"),
                image_processed,
                check_contrast=False,
            )
            imsave(
                str(debug_dir / f"{main_id}_6_final_mask.png"),
                mask_processed,
                check_contrast=False,
            )
...
def main():
    ...
    if not args.skip_analysis:
        ...
        if TARGET_SIZE < recommended_size:
            print(f"\n{'=' * 70}")
            print(
                f"[WARNING] Current TARGET_SIZE ({TARGET_SIZE}) is SMALLER than needed!"
            )
            print(f"{'=' * 70}")
            print(
    ...
    if orig_scales:
        ...
        print(f"{'=' * 70}")
        print(
            f"Original scales ranged from {min(orig_scales):.4f} to {max(orig_scales):.4f} um/px"
        )
