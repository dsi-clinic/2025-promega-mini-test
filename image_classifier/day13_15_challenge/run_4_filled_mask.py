...
from dataset_filled_mask import (
    OrganoidTimeSeriesDatasetFilledMask,
    SingleDayFilledMaskDataset,
)
...
def filter_ids_with_frames_up_to_day(organoid_ids, series_metadata, max_day):
    return [
        ...
        if any(d <= max_day for d in series_metadata.get(oid, {}).get("days", []))
    ]
...
def run_effnet_ts_filled_mask(
    ...
    if len(train_ids_f) == 0:
        print(
            f"No train organoids with frames <= day {day}, skipping effnet_ts filled_mask."
        )
        return None
    ...
    train_ds = OrganoidTimeSeriesDatasetFilledMask(
        train_ids_f, series_metadata, data, transform=train_tf, max_day=day
    )
    val_ds = OrganoidTimeSeriesDatasetFilledMask(
        val_ids_f, series_metadata, data, transform=eval_tf, max_day=day
    )
    test_ds = OrganoidTimeSeriesDatasetFilledMask(
        test_ids_f, series_metadata, data, transform=eval_tf, max_day=day
    )
    ...
    for epoch in range(1, EFFNET_MAX_EPOCHS + 1):
        ...
        for seqs, days_n, labels, weights, _ in tqdm(
            ...
            optimizer.step()
        val_loss, val_acc, _, _, _, _, _, _, _ = evaluate_effnet_ts(
    ...
    save_result = {
        "model_type": "effnet_ts_filled_mask",
        "day": day,
    ...
    print(
        f"Day {day} effnet_ts (filled_mask): test balanced_acc@0.5 = {test_at_05['balanced_acc']:.3f}"
    )
    return save_result


def run_per_day_filled_mask(
    day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir
):
    set_seed_base(SEED)
    ...
    train_ds = SingleDayFilledMaskDataset(
        train_ids, series_metadata, data, day, transform=train_tf
    )
    val_ds = SingleDayFilledMaskDataset(
        val_ids, series_metadata, data, day, transform=eval_tf
    )
    test_ds = SingleDayFilledMaskDataset(
        test_ids, series_metadata, data, day, transform=eval_tf
    )
    if len(train_ds) == 0:
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        with torch.no_grad():
            for imgs, labels, _ in val_loader:
                val_probs.extend(
                    torch.sigmoid(model(imgs.to(device))).cpu().numpy().ravel()
                )
    ...
    save_result = {
        "model_type": "per_day_filled_mask",
        "day": day,
        "best_val_acc": float(best_val_acc),
        "optimal_threshold": float(best_thresh),
        "test_at_0.5": test_at_05,
    ...
    with open(out_dir / "results.json", "w") as f:
        json.dump(save_result, f, indent=2)
    print(
        f"Day {day} per_day (filled_mask): test balanced_acc@0.5 = {test_at_05['balanced_acc']:.3f}"
    )
    return save_result
...
def main():
    ...
    for day in DAYS:
        run_effnet_ts_filled_mask(
            day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir
        )
        run_per_day_filled_mask(
            day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir
        )
