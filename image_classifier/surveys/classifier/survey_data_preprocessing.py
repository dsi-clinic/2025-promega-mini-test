...
def main():
    ...
    parser.add_argument(
        "--survey", default=SURVEY_JSON, help="Path to aggregated survey votes"
    )
    parser.add_argument(
        "--metadata", default=METADATA_JSON, help="Path to combined metadata JSON"
    )
    ...
    for obj_key, obj in survey.items():
        ...
        matched_keys = metadata_by_ba_well.get(
            (ba_full, well)
        ) or metadata_by_ba_well.get((ba_short, well))
        if not matched_keys:
            unmatched.append({"survey_object": obj_key, "image_id": raw_id})
            continue
    ...
    if unmatched:
        ...
        for entry in unmatched:
            print(
                f" • survey_object: {entry['survey_object']}  →  image_id: {entry['image_id']}"
            )
    else:
