...
for organoid, evaluations in data.items():
    ...
    for eval_item in evaluations:
        ...
        quality_counts[eval_item["quality"]]["employees"].append(
            employee_last_name
    ...
    if evaluation_count > 0:
        if class_counts:
            max_class_vote = (
                max(v["count"] for v in class_counts.values()) if class_counts else 0
            )
            agreement_ratio = (
    ...
    if evaluation_count > 1 and class_counts:
        ...
        majority_candidate_classes = [
            ...
            if data["count"] == max_vote_for_a_class
        ]
...
if stats["employee_dissent_stats"]:
    ...
    for emp, emp_data in sorted(
        stats["employee_dissent_stats"].items()
...
print(
    pd.DataFrame(
        expected, index=df_class_qual_counts.index, columns=df_class_qual_counts.columns
    )
)
