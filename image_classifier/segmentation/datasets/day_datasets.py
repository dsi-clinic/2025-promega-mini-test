...
class Dy30Dataset(BaseDataset):
        ...
        **kwargs,
        ...
        if not os.path.exists(self.json_mapping_path):
            raise FileNotFoundError(
                f"JSON mapping file does not exist: {self.json_mapping_path}"
            )

        # Initialize the dataset
        super().__init__(
            pipeline=pipeline, test_mode=test_mode, lazy_init=lazy_init, **kwargs
        )
    ...
    def load_data_list(self):
        ...
        if self.day_filter:
            ...
            print(
                f"Filtered to {len(image_mapping)} entries with dayID={self.day_filter}"
            )
