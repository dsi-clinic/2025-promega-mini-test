...
for img_id, img_info in filtered_mapping.items():
    ...
    try:
        ...
        if img is None:
            print(
                f"  -> Skipped: OpenCV could not load image (is it corrupted or wrong format?) at {img_path}"
            )
            skipped_count += 1
            continue
        img_shape = img.shape
        ...
        if mask is None:
            print(
                f"  -> Skipped: OpenCV could not load mask (is it corrupted or wrong format?) at {mask_path}"
            )
            skipped_count += 1
            continue
