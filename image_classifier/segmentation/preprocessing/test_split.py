...
DEFAULT_TRAIN_MAPPING = (
    TRAIN_MANUAL_PROCESSED_DIR / f"mapping_processed_total_{TARGET_SUFFIX}.json"
)
...
def split_and_save(
    full_map, out_prefix: str, split_dir: Path, train_frac: float, val_frac: float
):
...
def main(
    mapping_override: str | None, train_frac: float, val_frac: float, split_days: bool
):
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--mapping", default=None, help="Optional path to training mapping JSON"
    )
