...
from image_classifier.segmentation.mmseg_paths import (
    EARLY_MODEL,
    LATE_MODEL,
    OUTPUT_MASKS_BASE_DIR,
)
...
def run_inference(
    ...
    model = (
        None if dry_run else init_model(str(cfg_path), str(ckpt_path), device=device)
    )
    ...
    for json_mapping_path in mapping_paths:
        print(
            f"Checking for file: {json_mapping_path}  ->  {json_mapping_path.exists()}"
        )
        ...
        if batch_number == 2:
            ...
            output_dir = (
                Path(OUTPUT_MASKS_BASE_DIR)
                / f"batch{batch_number}_{part}"
                / f"day{day_str}"
            )
        else:
            output_dir = (
                Path(OUTPUT_MASKS_BASE_DIR) / f"batch{batch_number}" / f"day{day_str}"
            )
        ...
        sample_ids = (
            set(random.sample(all_ids, min(NUM_SAMPLES_FOR_COLLAGE, len(all_ids))))
            if all_ids
            else set()
        )
        ...
        for img_id, img_info in iter_items:
            ...
            try:
                ...
                pred_mask = (
                    result.pred_sem_seg.data.squeeze().cpu().numpy() * 255
                ).astype(np.uint8)
                ...
                if img_id in sample_ids:
                    ...
                    mask_vis = cv2.resize(
                        pred_mask, (img_w, img_h), interpolation=cv2.INTER_NEAREST
                    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--batches", type=lambda s: [int(x) for x in s.split(",")], required=True
    )
    ap.add_argument(
        "--days", type=lambda s: [int(x) for x in s.split(",")], required=True
    )
    ap.add_argument("--overwrite", action="store_true")
    ap.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate configs/paths without running inference",
    )
    ap.add_argument(
        "--smoke",
        type=int,
        default=None,
        help="Limit to N images per mapping for a quick test",
    )
    args = ap.parse_args()
