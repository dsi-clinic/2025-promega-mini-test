...
def main():
    parser = argparse.ArgumentParser(
        description="Create one chart and one 4-col table per model."
    )
    parser.add_argument(
        "--out-root",
        default="analysis/images/classifier/outputs_512x384_train_augment",
        help="Root directory with per-model day folders",
    )
    parser.add_argument(
        "--models",
        nargs="*",
        default=["vit", "resnet", "efficientnet"],
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Don’t print the tables to stdout"
    )
    ...
    for model in args.models:
        ...
        if not args.quiet:
            ...
            for r in rows:
                print(
                    f"{r['Day No']:>6} | {r['Num in Sample']:>13} | {r['Actual Good']:>11} | {r['Predicted Good']:>14}"
                )
            print()
