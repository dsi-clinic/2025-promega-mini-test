...
class OrganoidDataset(Dataset):
    """Dataset for organoid image classification with aggressive augmentation."""

    def __init__(self, img_paths, labels, augment=False):
        """Initialize organoid dataset.

        Args:
            img_paths: List of image file paths.
            labels: List of binary labels (0 or 1).
            augment: If True, apply aggressive data augmentation.
        """
...
class ImageOnlyClassifier(nn.Module):
    """Image classifier using EfficientNet backbone."""

    def __init__(self, backbone_name, target_size):
        """Initialize image classifier.

        Args:
            backbone_name: Backbone model name for timm (e.g., 'efficientnet_b0').
...
def evaluate_on_loader(model, loader):
    ...
    Args:
        ...
        loader: PyTorch DataLoader yielding (img, label) batches.

    Returns:
        Tuple of (preds_bin, trues, acc, f1, probs, tpr, tnr, balanced_acc).
    """
    model.eval()
...
def load_split_data(train_file: Path, val_file: Path, test_file: Path):
    ...
    Args:
        train_file: Path to train split JSON file.
        val_file: Path to validation split JSON file.
        test_file: Path to test split JSON file.
...
def extract_samples_by_day(split_data, day_str):
    ...
    Args:
        split_data: Dictionary from split JSON (organoid_id -> {label, timepoints, ...}).
        day_str: Day string (e.g., 'Dy03', 'Dy28').

    Returns:
        Tuple of (img_paths, labels) as numpy arrays.
    """
    img_paths = []
    labels = []
...
def run_training_for_day(
    ...
    """Train EfficientNet for one day with TNR-focused training.

    Uses balanced accuracy for model selection, boosted minority class weights,
    focal loss, and aggressive augmentation.

    Args:
        day_str: Day string to train on (e.g., 'Dy03', 'Dy28').
        ...
        val_bs: Validation/test batch size.
        out_root: Root output directory for saving models and metrics.

    Returns:
        Dictionary with test metrics, or None if insufficient data.
    """
    ...
    train_loader = make_loader(
        train_imgs, train_labels, augment=True, batch_size=train_bs
    )
    ...
    for epoch in range(100):
        ...
        print(
            f"[{day_str}][P1][{epoch:02d}] loss {tl:.4f}/{vl:.4f} | "
    ...
    for epoch in range(300):
        ...
        print(
            f"[{day_str}][P2][{epoch:03d}] loss {tl:.4f}/{vl:.4f} | "
            f"acc {tacc:.3f}/{vacc:.3f} | bal {tbal:.3f}/{vbal:.3f} | "
            f"TNR {ttnr:.3f}/{vtnr:.3f} | TPR {ttpr:.3f}/{vtpr:.3f}"
        )
    ...
    (
        ...
        val_tnr,
        val_balanced_acc,
    ) = evaluate_on_loader(model, val_loader)
    (
        preds_bin,
        test_trues,
        test_acc,
    ...
    try:
        float(roc_auc_score(val_trues, val_probs))
    except ValueError as e:
        # ROC-AUC may fail if all labels are the same class
        print(f"  WARNING: Could not compute val ROC-AUC: {e}")
    (
        float(average_precision_score(val_trues, val_probs))
        if len(val_trues) > 0
        else None
    )
...
# ---------- Main ----------
def main():
    """Entry point for TNR-optimized EfficientNet training.

    Trains EfficientNet models per day with balanced accuracy selection,
    boosted minority class weights, and aggressive augmentation.
    """
    set_seed()
    ...
    print("=" * 80)
    print("\nImprovements:")
    print("  [OK] Model selection by BALANCED ACCURACY (not accuracy)")
    print(f"  [OK] Minority class weight boosted by {MINORITY_CLASS_BOOST}x")
    print(f"  [OK] Focal loss alpha = {FOCAL_LOSS_ALPHA} (was 0.25)")
    print("  [OK] Track TNR/TPR during training")
    print("  [OK] Early stopping on balanced accuracy")
    print("  [OK] More aggressive augmentation")
    print("\nLoaded:")
    ...
    all_days = set()
    for org_data in (
        list(train_data.values()) + list(val_data.values()) + list(test_data.values())
    ):
