...
OUT_ROOT = Path(
    "image_classifier/training/outputs_512x384_softlabels_with_train_augment"
)
BATCH_SIZE = 16
...
def pick_threshold(
    y_true_bin: np.ndarray, y_prob: np.ndarray, method="max_f1", min_precision=None
) -> float:
...
def load_soft_records(day_json_path: Path, min_votes: int):
    ...
    for r in records:
        ...
        if (k is not None) and (n is not None):
            ...
            weights.append(
                float(n)
            )  # weight by number of votes (can be disabled later)
            ks.append(int(k))
            ns.append(int(n))
        else:
...
class OrganoidSoftDataset(Dataset):
    ...
    def __init__(self, img_paths, y_soft, weights=None, augment=False):
        ...
        self.y_soft = y_soft
        self.weights = (
            weights if weights is not None else np.ones_like(y_soft, dtype=np.float32)
        )
...
def run_training_for_day(
    ...
    imgs, y_soft, wts, k_votes, n_votes = load_soft_records(
        day_json_path, min_votes=min_votes
    )
    if len(imgs) == 0:
        print(
            f"⚠ Skipping {day_json_path.name} — no usable records (after min_votes={min_votes})"
        )
        return None
    ...
    for epoch in range(100):
        ...
        history["val_brier"].append(vl_brier)
        history["train_brier"].append(tbrier)
        history["val_rmse"].append(vl_dict["rmse"])
        print(
            f"[{day_json_path.stem}][{backbone_key}][P1][{epoch:02d}] "
    ...
    for epoch in range(300):
        ...
        history["val_rmse"].append(vl_dict["rmse"])
        print(
            f"[{day_json_path.stem}][{backbone_key}][P2][{epoch:03d}] "
            f"loss {tl:.4f} | brier {tbrier:.4f}/{vl_brier:.4f} | "
            f"rmse {tRMSE:.3f}/{vl_dict['rmse']:.3f} | acc@0.5 {tacc:.3f}/{vl_dict['acc@0.5']:.3f}"
        )
    ...
    plt.subplot(1, 2, 1)
    plt.plot(history["train_brier"], label="Train Brier")
    plt.plot(history["val_brier"], label="Val Brier")
    ...
    t_star = pick_threshold(
        val_true_bin, val_dict["probs"], method=th_method, min_precision=min_precision
    )
    ...
    print(
        f"⭐ Chosen threshold t*={t_star:.3f} via {th_method}"
        + (
            f" (min_precision={min_precision:.2f})"
            if (th_method == "prec_at_recall" and min_precision is not None)
            else ""
        )
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Minimum #votes required to include a sample (only used if raw_votes fields exist)",
    )
    parser.add_argument(
        "--weight-by-votes",
        action="store_true",
        help="Weight BCE loss by the sample's num_votes",
    )
    ...
    parser.add_argument(
        "--patience1", type=int, default=20, help="Early stopping patience (phase 1)"
    )
    parser.add_argument(
        "--patience2", type=int, default=30, help="Early stopping patience (phase 2)"
    )
    parser.add_argument(
        "--th-method",
        choices=["max_f1", "max_bal_acc", "prec_at_recall"],
        default="max_f1",
    ...
    for json_file in files:
        ...
        for backbone_key, backbone_name in BACKBONES.items():
            res = run_training_for_day(
                ...
                lr1=args.lr1,
                lr2=args.lr2,
                patience1=args.patience1,
                patience2=args.patience2,
    ...
    for d in days_sorted:
        ...
        rows.append(
            {
                "Day No": r["day_no"]
                / 100.0,  # undo *100 scaling for human-friendly display
    ...
    if day_numbers:
        ...
        plot_metric(
            "test_brier",
            "Brier score (test) ↓",
            "Per-day Test Brier by Backbone",
            "brier_by_model.png",
            bounded=False,
        )
        plot_metric(
        ...
        plot_metric(
            "test_roc_auc",
            "ROC AUC (test)",
            "Per-day Test ROC AUC by Backbone",
            "rocauc_by_model.png",
            bounded=True,
        )
        ...
        per_model_summary[backbone_key] = {
            "per_day": {
                day: {
                    ...
                    "test_roc_auc": (
                        None
                        if res.get("test_roc_auc") is None
                        else float(res["test_roc_auc"])
                    ),
                    "test_pr_auc": (
                        None
                        if res.get("test_pr_auc") is None
                        else float(res["test_pr_auc"])
                    ),
                    "val_roc_auc": (
                        None
                        if res.get("val_roc_auc") is None
                        else float(res["val_roc_auc"])
                    ),
                    "val_pr_auc": (
                        None
                        if res.get("val_pr_auc") is None
                        else float(res["val_pr_auc"])
                    ),
    ...
    summary = {
        ...
        "settings": {
            ...
            "min_precision": (
                None if args.min_precision is None else float(args.min_precision)
            ),
        },
    }
    ...
    print(
        f"{'Day':>6} | {'Num in Sample':>13} | {'Actual Good':>12} | {'Pred Good':>10}"
    )
    print("-" * 56)
    for row in rows:
        print(
            f"{row['Day No']:>6} | {row['Num in Sample']:>13} | {row['Actual Good@0.5']:>12} | {row['Predicted Good@0.5']:>10}"
        )
