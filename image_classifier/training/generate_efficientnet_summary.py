...
def main():
    ...
    parser.add_argument(
        "--results-dir", required=True, help="Directory with training results"
    )
    parser.add_argument(
        "--split-prefix",
        required=True,
        help="Prefix for split files (e.g., 'both_train_base_no_stitch')",
    )
    parser.add_argument(
        "--output-name", required=True, help="Output CSV filename (without .csv)"
    )
    ...
    for day_dir in sorted(efficientnet_dir.iterdir()):
        ...
        row = {
            ...
            "Test_ROC_AUC": f"{metrics.get('roc_auc', 0):.4f}"
            if metrics.get("roc_auc")
            else "N/A",
            "Test_PR_AUC": f"{metrics.get('pr_auc', 0):.4f}"
    ...
    if summary_rows:
        ...
        avg_balanced = sum(
            float(r["Test_Balanced_Accuracy"]) for r in summary_rows
        ) / len(summary_rows)
