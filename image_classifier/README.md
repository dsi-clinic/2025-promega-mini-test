...
Scripts under `image_classifier/` (training, comparison_runs, regeneration) use `PROJ_ROOT` or the placeholder `/home/YOUR_GITHUB_USERNAME/promega-classifier` for the repo root. See the [root README](../README.md#path-configuration) for setup details. For SLURM jobs:

```bash
export PROJ_ROOT=/home/yourname/promega-classifier
sbatch --export=PROJ_ROOT image_classifier/comparison_runs/submit_per_day_study_effnet_ts.slurm
```
