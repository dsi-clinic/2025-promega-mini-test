...
| SLURM script | `analysis/legacy_paper_2026_04/run_perday_image_study.sh` |
...
| SLURM script | `analysis/images/cnn_lstm/run_temporal_ablation_attn.sh` |
...
## Mean-Filled vs Raw Image Comparison (2026-05-27)

Both models were re-run with `cm_image_abs` (pre-computed mean-filled AR-conserved images
from `mean_fill_clip/`) to compare against the raw `cm_source_image_abs` baseline.

### Per-Day EfficientNet

Mean-filled performance is essentially equivalent to raw (avg bal acc 59.7% vs 59.8%).
Small shuffles at individual days with no clear directional trend. **Conclusion: image type
does not meaningfully affect per-day model performance.**

### CNN-LSTM Temporal Ablation

The mean-filled run collapses completely: best test accuracy across all nine day ranges
is 0.154 with recall=1.0, meaning the model predicts every organoid as Not Acceptable.
The per-day model is unaffected, so the images do contain sufficient signal — the failure
is specific to the sequential setting.

Coverage is not the cause: every record has both `cm_source_image_abs` and `cm_image_abs`
(2178/2178). The most likely cause is that zero-padded frames (used for variable-length
sequences) interact poorly with mean-filled images during training, pushing the model
toward the trivial all-positive solution before it can learn. The old `series_idor`
mean-filled run (AUC 0.791) did not exhibit this because it required complete 11-day
series with no padding.

**Conclusion: raw `cm_source_image_abs` images are used for both models in the final
submission. The mean-filled CNN-LSTM result is not reported.**

---

## Open Questions

1. **Time series peaks at Day 28, not Day 30**: Adding the final two days hurts the sequential
...
2. **2 missing test organoids**: The CNN-LSTM test set has 39 vs 41 for per-day. Two canonical
...
| `461f614` | Add `cm_image` image type support to CNN-LSTM for mean-filled comparison |
| `4dc26f2` | Suffix per-day output filenames with input mode to prevent overwrites |
| `fec1427` | Filter organoids without clipped images before training |
| `27390a5` | Skip None image paths in `__getitem__` to prevent empty sequence crash |
