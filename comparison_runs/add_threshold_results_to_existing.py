...
def add_threshold_results_per_day(out_dir, data, device, image_key="overlay_path", in_channels=3):
    ...
    val_ds = SingleDayOrganoidDataset(val_ids, series_metadata, data_dict, day, transform=eval_tf,
                                      image_key=image_key, use_rgb_mask=False)
    test_ds = SingleDayOrganoidDataset(test_ids, series_metadata, data_dict, day, transform=eval_tf,
                                       image_key=image_key, use_rgb_mask=False)
    ...
    return build_threshold_results(val_probs, val_labels, test_probs, test_labels, best_thresh)


def add_threshold_results_effnet_ts(out_dir, data, device, image_key="overlay_path", in_channels=3):
    ...
    if not (out_dir / "best_model.pth").exists() or not (out_dir / "global_mean.npy").exists():
        return None
    ...
    val_ds = OrganoidTimeSeriesDataset(val_ids_f, series_metadata, data_dict, global_mean=global_mean, max_day=day,
                                       image_key=image_key, input_rgb_mask=False)
    test_ds = OrganoidTimeSeriesDataset(test_ids_f, series_metadata, data_dict, global_mean=global_mean, max_day=day,
                                        image_key=image_key, input_rgb_mask=False)
    val_loader = DataLoader(val_ds, batch_size=EFFNET_TS_BATCH, shuffle=False, num_workers=NUM_WORKERS,
                            collate_fn=collate_variable_length)
    test_loader = DataLoader(test_ds, batch_size=EFFNET_TS_BATCH, shuffle=False, num_workers=NUM_WORKERS,
                             collate_fn=collate_variable_length)
...
def main():
    ...
    data = {
        ...
        "series_metadata": series_metadata, "data": data_dict,
    }
        ...
        for day_dir in path.iterdir():
            ...
            try:
                if model_type == "per_day":
                    tr = add_threshold_results_per_day(day_dir, data, device, image_key=image_key, in_channels=in_channels)
                else:
                    tr = add_threshold_results_effnet_ts(day_dir, data, device, image_key=image_key, in_channels=in_channels)
                if tr is None:
