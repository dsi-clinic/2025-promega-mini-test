...
# Env and path setup (same as tune_thresholds)
_script_parent = Path(__file__).resolve().parents[1]
_repo_dir = _script_parent / "2025-promega-mini-test"
env_file = _repo_dir / ".env" if _repo_dir.is_dir() else _script_parent / ".env"
if env_file.exists():
...
ROOT = (_script_parent / "2025-promega-mini-test") if _repo_dir.is_dir() else _script_parent
sys.path.insert(0, str(ROOT))

from analysis.images.cnn_lstm.load_split_data import load_split_data
from analysis.images.cnn_lstm.organoid_dataset import OrganoidTimeSeriesDataset, compute_global_mean_from_ids
...
def build_threshold_results(val_probs, val_labels, test_probs, test_labels, best_thresh,
                            train_probs=None, train_labels=None):
...
def run_per_day(day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir,
                image_key='img_path', use_rgb_mask=False, in_channels=3, save_model=True):
    ...
    result = train_for_day(day, train_ids, val_ids, test_ids, series_metadata, data, device, out_dir,
                          image_key=image_key, use_rgb_mask=use_rgb_mask, in_channels=in_channels, save_model=save_model)
    if result is None:
        return None
    from analysis.images.cnn_lstm.train_base_model import BaselineEfficientNet, evaluate
    eval_tf = T.Compose([T.Resize(TARGET_SIZE)])
    train_ds = SingleDayOrganoidDataset(train_ids, series_metadata, data, day, transform=eval_tf,
                                        image_key=image_key, use_rgb_mask=use_rgb_mask)
    val_ds = SingleDayOrganoidDataset(val_ids, series_metadata, data, day, transform=eval_tf,
                                      image_key=image_key, use_rgb_mask=use_rgb_mask)
    test_ds = SingleDayOrganoidDataset(test_ids, series_metadata, data, day, transform=eval_tf,
                                       image_key=image_key, use_rgb_mask=use_rgb_mask)
    ...
    threshold_results = build_threshold_results(val_probs, val_labels, test_probs, test_labels, best_thresh,
                                                train_probs=train_probs, train_labels=train_labels_arr)
    save_result = {
        "model_type": "per_day",
        "day": day,
...
def run_cnn_lstm_accumulated(day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir,
                             image_key='image_path', input_rgb_mask=False, in_channels=3, save_model=True):
    ...
    if len(train_ids_f) == 0:
        ...
        return None
    print(f"Using {len(train_ids_f)}/{len(train_ids)} train, {len(val_ids_f)}/{len(val_ids)} val, {len(test_ids_f)}/{len(test_ids)} test (frames <= {day})")
    ...
    eval_tf = T.Compose([T.Resize((384, 384), interpolation=InterpolationMode.BILINEAR)])
    train_ds = OrganoidTimeSeriesDataset(train_ids_f, series_metadata, data, global_mean=global_mean, max_day=day,
                                        image_key=image_key, input_rgb_mask=input_rgb_mask, transform=train_tf)
    val_ds = OrganoidTimeSeriesDataset(val_ids_f, series_metadata, data, global_mean=global_mean, max_day=day,
                                       image_key=image_key, input_rgb_mask=input_rgb_mask, transform=eval_tf)
    test_ds = OrganoidTimeSeriesDataset(test_ids_f, series_metadata, data, global_mean=global_mean, max_day=day,
                                        image_key=image_key, input_rgb_mask=input_rgb_mask, transform=eval_tf)
    g = torch.Generator().manual_seed(SEED)
    train_loader = DataLoader(train_ds, batch_size=8, shuffle=True, num_workers=4, collate_fn=collate_variable_length, generator=g)
    val_loader = DataLoader(val_ds, batch_size=8, shuffle=False, num_workers=4, collate_fn=collate_variable_length)
    test_loader = DataLoader(test_ds, batch_size=8, shuffle=False, num_workers=4, collate_fn=collate_variable_length)
    ...
    model = OrganoidCNN_LSTM(num_classes=2, lstm_hidden=256, lstm_layers=2, in_channels=in_channels).to(device)
    ...
    for epoch in range(20):
        ...
        val_loss, val_acc, val_prec, val_rec, val_f1, _, _ = evaluate_cnn_lstm(model, val_loader, criterion, device)
        scheduler.step(val_loss)
        train_history.append({"epoch": epoch + 1, "train_loss": train_loss, "train_acc": train_acc, "val_loss": val_loss, "val_acc": val_acc, "val_precision": val_prec, "val_recall": val_rec, "val_f1": val_f1})
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            if save_model:
                torch.save({"model_state_dict": model.state_dict(), "val_acc": val_acc}, out_dir / "best_model.pth")
            else:
                best_state_dict = {k: v.cpu().clone() for k, v in model.state_dict().items()}
    if save_model:
        ckpt = torch.load(out_dir / "best_model.pth", map_location=device)
    else:
        ckpt = {"model_state_dict": best_state_dict or model.state_dict(), "val_acc": best_val_acc}
...
def run_effnet_ts_accumulated(day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir,
                              image_key='image_path', input_rgb_mask=False, in_channels=3, save_model=True):
    ...
    np.save(out_dir / "global_mean.npy", global_mean)
    from torchvision.transforms import InterpolationMode
    train_tf = T.Compose([T.Resize((384, 384), interpolation=InterpolationMode.BILINEAR), T.RandomHorizontalFlip(0.5), T.RandomVerticalFlip(0.5), T.ColorJitter(brightness=0.2, contrast=0.2, saturation=0, hue=0)])
    eval_tf = T.Compose([T.Resize((384, 384), interpolation=InterpolationMode.BILINEAR)])
    train_ds = OrganoidTimeSeriesDataset(train_ids_f, series_metadata, data, global_mean=global_mean, max_day=day, image_key=image_key, input_rgb_mask=input_rgb_mask, transform=train_tf)
    val_ds = OrganoidTimeSeriesDataset(val_ids_f, series_metadata, data, global_mean=global_mean, max_day=day, image_key=image_key, input_rgb_mask=input_rgb_mask, transform=eval_tf)
    test_ds = OrganoidTimeSeriesDataset(test_ids_f, series_metadata, data, global_mean=global_mean, max_day=day, image_key=image_key, input_rgb_mask=input_rgb_mask, transform=eval_tf)
    g = torch.Generator().manual_seed(SEED)
    train_loader = DataLoader(train_ds, batch_size=EFFNET_TS_BATCH, shuffle=True, num_workers=NUM_WORKERS, collate_fn=collate_variable_length, pin_memory=(device.type=="cuda"), generator=g)
    val_loader = DataLoader(val_ds, batch_size=EFFNET_TS_BATCH, shuffle=False, num_workers=NUM_WORKERS, collate_fn=collate_variable_length, pin_memory=(device.type=="cuda"))
    test_loader = DataLoader(test_ds, batch_size=EFFNET_TS_BATCH, shuffle=False, num_workers=NUM_WORKERS, collate_fn=collate_variable_length, pin_memory=(device.type=="cuda"))
    train_labels = [1 if str(series_metadata[oid].get("label","")).strip().lower() in ("good","acceptable","accepted") else 0 for oid in train_ids_f]
    ...
    model = OrganoidCNN_TAtt(attn_dropout=ATTN_DROPOUT, in_channels=in_channels).to(device)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        val_loss, val_acc, val_prec, val_rec, val_f1, _, _, _, _ = evaluate_effnet_ts(model, val_loader, criterion, device)
        scheduler.step(val_loss)
        train_history.append({"epoch": epoch, "train_loss": train_loss, "train_acc": train_acc, "val_loss": val_loss, "val_acc": val_acc, "val_precision": val_prec, "val_recall": val_rec, "val_f1": val_f1})
    ...
    train_eval_ds = OrganoidTimeSeriesDataset(train_ids_f, series_metadata, data, global_mean=global_mean, max_day=day,
                                              image_key=image_key, input_rgb_mask=input_rgb_mask, transform=eval_tf)
    train_eval_loader = DataLoader(train_eval_ds, batch_size=EFFNET_TS_BATCH, shuffle=False,
    ...
    threshold_results = build_threshold_results(val_probs, val_labels, test_probs, test_labels, best_thresh,
                                                train_probs=train_probs, train_labels=train_labels_arr)
    train_predictions = [{"organoid_id": oid, "label": int(l), "prob": float(p)} for oid, l, p in zip(train_organoid_ids, train_labels_arr, train_probs)]
    val_predictions = [{"organoid_id": oid, "label": int(l), "prob": float(p)} for oid, l, p in zip(val_organoid_ids, val_labels, val_probs)]
    test_predictions = [{"organoid_id": oid, "label": int(l), "prob": float(p)} for oid, l, p in zip(test_organoid_ids, test_labels, test_probs)]
    save_result = {
...
def run_effnet_tchange_accumulated(day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir,
                                    image_key='image_path', input_rgb_mask=False, in_channels=3, save_model=True):
    ...
    if len(train_ids_f) == 0:
        print(f"No train organoids with frames <= day {day}, skipping.")
        return None
    print(f"[tchange] {len(train_ids_f)}/{len(train_ids)} train, {len(val_ids_f)}/{len(val_ids)} val, {len(test_ids_f)}/{len(test_ids)} test (frames <= {day})")
    global_mean = compute_global_mean_from_ids(train_ids_f, series_metadata, data)
    np.save(out_dir / "global_mean.npy", global_mean)
    from torchvision.transforms import InterpolationMode
    train_tf = T.Compose([T.Resize((384, 384), interpolation=InterpolationMode.BILINEAR), T.RandomHorizontalFlip(0.5), T.RandomVerticalFlip(0.5), T.ColorJitter(brightness=0.2, contrast=0.2, saturation=0, hue=0)])
    ...
    w_neg = n_total / (2.0 * max(n_bad, 1))
    criterion = nn.BCEWithLogitsLoss(reduction="none")
    model = OrganoidCNN_TChange(attn_dropout=ATTN_DROPOUT, in_channels=in_channels).to(device)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        model.train()
        running_loss, correct, total = 0.0, 0, 0
        for seqs, days_n, labels, weights, _ in tqdm(train_loader, desc=f"[tchange] Epoch {epoch}", leave=False):
            seqs, days_n = seqs.to(device), days_n.to(device).float()
            labels, weights = labels.to(device).float(), weights.to(device).float()
            optimizer.zero_grad()
            logits, _ = model(seqs, days_n)
        ...
        train_acc = correct / max(total, 1)
        train_loss = running_loss / max(total, 1)
        val_loss, val_acc, val_prec, val_rec, val_f1, _, _, _, _ = evaluate_tchange(model, val_loader, criterion, device)
        scheduler.step(val_loss)
        train_history.append({"epoch": epoch, "train_loss": train_loss, "train_acc": train_acc, "val_loss": val_loss, "val_acc": val_acc, "val_precision": val_prec, "val_recall": val_rec, "val_f1": val_f1})
        print(f"  [tchange] Ep {epoch:02d} train_acc={train_acc:.3f} val_acc={val_acc:.3f} val_f1={val_f1:.3f}")
        if val_acc > best_val_acc + 1e-4:
            best_val_acc, best_state = val_acc, {k: v.cpu().clone() for k, v in model.state_dict().items()}
            bad_epochs = 0
        else:
...
def results_to_csv_rows(setup, fold, model_type, day, optimal_threshold, threshold_results, split="test"):
...
def main():
    parser = argparse.ArgumentParser(description="Per-day study: one of per_day, cnn_lstm, effnet_ts, effnet_tchange at one day")
    parser.add_argument("--model_type", type=str, required=True, choices=["per_day", "cnn_lstm", "effnet_ts", "effnet_tchange"])
    parser.add_argument("--day", type=float, required=True, help="Target day (e.g. 6, 8, 20.5, 30)")
    parser.add_argument("--input_mode", type=str, default="rgb", choices=["rgb", "overlay", "rgb_mask"],
    ...
    print(f"Model: {args.model_type}, day: {args.day}, input_mode: {args.input_mode}, base_dir: {base_dir}")
    print(f"  data: train={len(train_ids)} val={len(val_ids)} test={len(test_ids)}")
    if args.model_type == "per_day":
        run_per_day(args.day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir,
                    image_key=image_key, use_rgb_mask=use_rgb_mask, in_channels=in_channels)
    elif args.model_type == "cnn_lstm":
        run_cnn_lstm_accumulated(args.day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir,
                                image_key=image_key, input_rgb_mask=input_rgb_mask, in_channels=in_channels)
    elif args.model_type == "effnet_ts":
        run_effnet_ts_accumulated(args.day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir,
                                 image_key=image_key, input_rgb_mask=input_rgb_mask, in_channels=in_channels)
    else:
        run_effnet_tchange_accumulated(args.day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir,
                                       image_key=image_key, input_rgb_mask=input_rgb_mask, in_channels=in_channels)
    print("Done.")
