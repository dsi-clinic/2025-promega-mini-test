...
_repo_dir = _tp / "2025-promega-mini-test"
env_file = _repo_dir / ".env" if _repo_dir.is_dir() else _tp / ".env"
if env_file.exists():
...
os.environ.setdefault("RAW_IMAGE_DATA", "/net/projects2/promega/data-analysis/raw-images")
os.environ.setdefault("IMAGE_VERIFICATION_FORM", "/net/projects2/promega/data-analysis/image-verification-form.json")
...
os.environ.setdefault("PREDICTIONS_DIR", "/net/projects2/promega/data-analysis/predictions")
os.environ.setdefault("SURVEY_RESULTS", "/net/projects2/promega/data-analysis/survey-results")
os.environ.setdefault("MANUAL_MASKS_DIR", "/net/projects2/promega/data-analysis/manual-masks")
os.environ.setdefault("META_FILE", "/net/projects2/promega/data-analysis/metadata.json")
os.environ.setdefault("RAW_IMAGE_MAPPING_JSON", "/net/projects2/promega/data-analysis/image-mapping.json")
os.environ.setdefault("TARGET_WIDTH", "512")
os.environ.setdefault("TARGET_HEIGHT", "384")
os.environ.setdefault("TRAIN_RESIZED_DIR", "/net/projects2/promega/data-analysis/train-resized")
os.environ.setdefault("TRAIN_MANUAL_MAPPING_DIR", "/net/projects2/promega/data-analysis/train-mapping")
os.environ.setdefault("TRAIN_MANUAL_PROCESSED_DIR", "/net/projects2/promega/data-analysis/train-processed")
os.environ.setdefault("TRAIN_SPLITS_DIR", "/net/projects2/promega/data-analysis/train-splits")
os.environ.setdefault("INFER_RESIZED_DIR", "/net/projects2/promega/data-analysis/infer-resized")
os.environ.setdefault("INFER_MAPPING_TOTAL_JSON", "/net/projects2/promega/data-analysis/infer-mapping.json")
os.environ.setdefault("MANUAL_THRESHOLD_MAPPING", "/net/projects2/promega/data-analysis/threshold-mapping.json")

# Add project root to path (works when script is in repo or in parent workspace)
ROOT = _repo_dir if _repo_dir.is_dir() else _tp
...
from analysis.images.cnn_lstm.train_base_model import SingleDayOrganoidDataset
...
def main():
    ...
    if not cnn_lstm_model_path.exists():
        ...
        val_dataset = OrganoidTimeSeriesDataset(val_ids, series_metadata, data, global_mean=global_mean)
        val_loader = DataLoader(val_dataset, batch_size=8, shuffle=False, num_workers=4, collate_fn=collate_variable_length)
        ...
        print(f"  Confusion Matrix: TN={best_results['TN']}, FP={best_results['FP']}, FN={best_results['FN']}, TP={best_results['TP']}")
        
        # Evaluate on test set with optimal threshold
        print("\nEvaluating test set with optimal threshold...")
        test_dataset = OrganoidTimeSeriesDataset(test_ids, series_metadata, data, global_mean=global_mean)
        test_loader = DataLoader(test_dataset, batch_size=8, shuffle=False, num_workers=4, collate_fn=collate_variable_length)
        test_probs, test_labels = get_cnn_lstm_probs(model, test_loader, device)
        test_results = calculate_metrics_at_threshold(test_probs, test_labels, best_thresh)
        ...
        print(f"  Confusion Matrix: TN={test_results['TN']}, FP={test_results['FP']}, FN={test_results['FN']}, TP={test_results['TP']}")
    ...
    if not effnet_model_path.exists():
        ...
        val_dataset = SingleDayOrganoidDataset(val_ids, series_metadata, data, 30, transform=eval_tf)
        ...
        # Evaluate on test set with optimal threshold
        print("\nEvaluating test set with optimal threshold...")
        test_dataset = SingleDayOrganoidDataset(test_ids, series_metadata, data, 30, transform=eval_tf)
    ...
    if not effnet_ts_model_path.exists():
        ...
        from analysis.images.cnn_lstm.train_temporal_ablation_attn import OrganoidCNN_TAtt
        ...
        val_dataset_ts = OrganoidTimeSeriesDataset(val_ids, series_metadata, data, global_mean=global_mean_ts)
        val_loader_ts = DataLoader(val_dataset_ts, batch_size=8, shuffle=False, num_workers=4, collate_fn=collate_variable_length)
        print("Getting predictions on validation set...")
        val_probs, val_labels = get_effnet_timeseries_probs(model, val_loader_ts, device)
        print(f"  Validation set: {len(val_probs)} samples")
        ...
        print(f"  TNR: {best_results['TNR']:.4f} ({best_results['TNR']*100:.1f}%)")
        print(f"  TPR: {best_results['TPR']:.4f} ({best_results['TPR']*100:.1f}%)")
        print(f"  Accuracy: {best_results['accuracy']:.4f} | F1: {best_results['f1']:.4f}")
        print(f"  Confusion Matrix: TN={best_results['TN']}, FP={best_results['FP']}, FN={best_results['FN']}, TP={best_results['TP']}")
        
        test_dataset_ts = OrganoidTimeSeriesDataset(test_ids, series_metadata, data, global_mean=global_mean_ts)
        test_loader_ts = DataLoader(test_dataset_ts, batch_size=8, shuffle=False, num_workers=4, collate_fn=collate_variable_length)
        test_probs, test_labels = get_effnet_timeseries_probs(model, test_loader_ts, device)
        test_results = calculate_metrics_at_threshold(test_probs, test_labels, best_thresh)
        print(f"\nTest Set Results (threshold={best_thresh:.3f}):")
        print(f"  TNR: {test_results['TNR']:.4f} ({test_results['TNR']*100:.1f}%)")
        print(f"  TPR: {test_results['TPR']:.4f} ({test_results['TPR']*100:.1f}%)")
        print(f"  Accuracy: {test_results['accuracy']:.4f} | F1: {test_results['f1']:.4f}")
        print(f"  Confusion Matrix: TN={test_results['TN']}, FP={test_results['FP']}, FN={test_results['FN']}, TP={test_results['TP']}")
    ...
    if cnn_lstm_results:
        ...
        print(f"  Test TNR: {cnn_lstm_results['test_results']['TNR']:.4f} ({cnn_lstm_results['test_results']['TNR']*100:.1f}%)")
        print(f"  Test F1: {cnn_lstm_results['test_results']['f1']:.4f}")
    if effnet_results:
        print(f"\nEfficientNet:")
        print(f"  Optimal threshold: {effnet_results['optimal_threshold']:.3f}")
        print(f"  Test TNR: {effnet_results['test_results']['TNR']:.4f} ({effnet_results['test_results']['TNR']*100:.1f}%)")
        print(f"  Test F1: {effnet_results['test_results']['f1']:.4f}")
    if effnet_timeseries_results:
        print(f"\nEfficientNet Time-Series:")
        print(f"  Optimal threshold: {effnet_timeseries_results['optimal_threshold']:.3f}")
        print(f"  Test TNR: {effnet_timeseries_results['test_results']['TNR']:.4f} ({effnet_timeseries_results['test_results']['TNR']*100:.1f}%)")
    ...
    if effnet_results:
        ...
        print(f"  EfficientNet (1-day):  Val TNR {vr['TNR']*100:.1f}%  →  Test TNR {tr['TNR']*100:.1f}%  (TN={tr['TN']}, FP={tr['FP']}, 9 Bad in test)")
    if effnet_timeseries_results:
        vr, tr = effnet_timeseries_results['val_results'], effnet_timeseries_results['test_results']
        print(f"  EfficientNet (TS):     Val TNR {vr['TNR']*100:.1f}%  →  Test TNR {tr['TNR']*100:.1f}%  (TN={tr['TN']}, FP={tr['FP']}, 9 Bad in test)")
    print()
