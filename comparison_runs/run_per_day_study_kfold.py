...
_repo_dir = COMPARISON_RUNS.parent / "2025-promega-mini-test"
env_file = _repo_dir / ".env" if _repo_dir.is_dir() else COMPARISON_RUNS.parent / ".env"
if env_file.exists():
...
ROOT = _repo_dir if _repo_dir.is_dir() else COMPARISON_RUNS.parent
...
def make_kfold_splits(train_ids, val_ids, series_metadata, k=5, seed=42):
    ...
    good = lambda oid: str(series_metadata.get(oid, {}).get("label", "")).strip().lower() in ("good", "acceptable", "accepted")
...
def main():
    parser = argparse.ArgumentParser(description="5-fold CV per-day study, one run per array index")
    parser.add_argument("--array_index", type=int, required=True, help=f"0 to {TOTAL-1}")
    ...
    print(f"kfold run: fold={fold} setup={setup_name} model={model_type} day={day} array_index={args.array_index}")
    print(f"  train={len(train_ids_f)} val={len(val_ids_f)} test={len(test_ids)}")

    if model_type == "per_day":
        run_per_day(day, train_ids_f, val_ids_f, test_ids, series_metadata, data, device, base_dir,
                    image_key=image_key, use_rgb_mask=use_rgb_mask, in_channels=in_channels)
    elif model_type == "cnn_lstm":
        run_cnn_lstm_accumulated(day, train_ids_f, val_ids_f, test_ids, series_metadata, data, device, base_dir,
                                image_key=image_key, input_rgb_mask=input_rgb_mask, in_channels=in_channels)
    else:
        run_effnet_ts_accumulated(day, train_ids_f, val_ids_f, test_ids, series_metadata, data, device, base_dir,
                                 image_key=image_key, input_rgb_mask=input_rgb_mask, in_channels=in_channels)
    print("Done.")
