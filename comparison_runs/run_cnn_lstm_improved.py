...
from analysis.images.cnn_lstm.train_organoid_lstm_single_phase import (
    train_one_epoch, set_seed, collate_variable_length
)
...
def evaluate_with_threshold(model, dataloader, criterion, device, threshold=0.5):
    ...
    return avg_loss, accuracy, precision, recall, f1, TNR, balanced_acc, preds, all_labels

def main():
    import argparse
    parser = argparse.ArgumentParser(description='Train CNN-LSTM with improved class weights and threshold')
    ...
    parser.add_argument('--weight-multiplier', type=float, default=1.27, help='Multiplier for class weights (1.27 gives ~5x penalty)')
    parser.add_argument('--threshold', type=float, default=0.460, help='Classification threshold (from tuning)')
    ...
    # Output directory
    output_dir = Path(__file__).parent / "amanda_cnn_lstm_improved"
    ...
    weight_for_0 = (len(train_labels) / (2 * n_bad)) * args.weight_multiplier if n_bad > 0 else 1.0
    ...
    print(f"Early stopping based on balanced accuracy (TNR + TPR) / 2")
    ...
    for epoch in range(args.epochs):
        ...
        val_loss, val_acc, val_prec, val_rec, val_f1, val_tnr, val_balanced_acc, _, _ = evaluate_with_threshold(
        ...
        print(f"Epoch {epoch+1:2d}/{args.epochs}: Train {train_acc:.3f} | Val {val_acc:.3f} | "
        ...
        if val_balanced_acc > best_balanced_acc:
            ...
            print(f"  *** Best Balanced Acc: {val_balanced_acc:.4f} (TNR: {val_tnr:.4f})")
        else:
            patience_counter += 1
    ...
    test_loss, test_acc, test_precision, test_recall, test_f1, test_tnr, test_balanced_acc, test_preds, test_labels = evaluate_with_threshold(
    ...
    results = {
        ...
        'class_weights': {'bad': float(weight_for_0), 'good': float(weight_for_1), 'ratio': float(penalty_ratio)},
