...
ROOT = (COMPARISON_RUNS.parent / "2025-promega-mini-test") if (COMPARISON_RUNS.parent / "2025-promega-mini-test").is_dir() else COMPARISON_RUNS.parent
# Load .env first
env_file = ROOT / ".env"
if env_file.exists():
...
from run_per_day_study import DAYS_11, run_per_day, run_effnet_ts_accumulated, run_effnet_tchange_accumulated
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Overlay-only threshold study: per_day + effnet_ts, 11 days, then export CSV")
    p.add_argument("--save_model", action="store_true", help="Save .pth checkpoints (default: no, to keep disk <2GB)")
    p.add_argument("-o", "--output", default=None, help="Output CSV (default: comparison_runs/overlay_threshold_study_results.csv)")
    p.add_argument("--output_dir", default=None, help="If set, write per_day_study_overlay and CSV under this dir (for parallel runs)")
    p.add_argument("--models", default="both", choices=["both", "per_day", "effnet_ts", "effnet_tchange"], help="Which models to run (default: both)")
    p.add_argument("--force", action="store_true", help="Re-run even if results.json already exists (needed for adding train metrics to existing dirs)")
    p.add_argument("--export_predictions", action="store_true", help="Export per-organoid predictions to predictions_Dy*.csv (organoid_id, split, day, y_true, y_pred, y_score)")
    args = p.parse_args()
    ...
    if output_dir is not None:
        ...
        out_csv = Path(args.output) if args.output else COMPARISON_RUNS / "overlay_threshold_study_results.csv"
        ...
        for day in DAYS_11:
            ...
            if model_type == "per_day":
                ...
                run_effnet_tchange_accumulated(
                    day, train_ids, val_ids, test_ids, series_metadata, data, device, base_dir,
                    image_key=image_key, input_rgb_mask=input_rgb_mask, in_channels=in_channels, save_model=save_model,
                )
            else:
    ...
    all_rows = collect_rows(include_kfold=False, setup_filter="overlay", overlay_base=base_dir,
                            splits=["train", "val", "test"])
    if all_rows:
        all_csv = out_csv.parent / out_csv.name.replace(".csv", "_all_splits.csv")
        with open(all_csv, "w", newline="") as f:
            w = csv.DictWriter(f, fieldnames=CSV_COLUMNS_WITH_SPLIT, extrasaction="ignore")
            w.writeheader()
